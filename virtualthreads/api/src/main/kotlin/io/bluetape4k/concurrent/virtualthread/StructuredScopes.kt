 ...
 */
interface StructuredTaskScopeFirst<T>: AutoCloseable {
    fun <V: T> fork(task: () -> V): StructuredSubtask<V>
    fun join(): StructuredTaskScopeFirst<T>
    fun result(mapper: (Throwable) -> RuntimeException): T
    override fun close()
}

/**
 * `withFirst`의 의미를 명확히 하기 위한 별칭입니다.
 */
typealias StructuredTaskScopeAny<T> = StructuredTaskScopeFirst<T>
