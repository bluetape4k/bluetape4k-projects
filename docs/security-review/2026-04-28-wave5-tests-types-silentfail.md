...
- **설계 트레이드오프 (사용자
  확인):** `null` 반환 방식은 Consumer에서 offset 관리를 단순화하는 의도적 선택. 단, Kafka 압축 시 tombstone (의도적 삭제)과 구분 불가. DLQ 라우팅 필요 시 `SerializationException` throw 방식으로 전환 필요.
- **현재
  방향:** `null` 반환 유지 시 최소한 `.onFailure { log.warn(it) { "deserialization failed" } }` 추가하여 poison pill 발생을 로그로 관찰 가능하게 할 것.
