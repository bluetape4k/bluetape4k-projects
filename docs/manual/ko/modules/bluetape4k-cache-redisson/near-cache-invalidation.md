...
1.11.0 구현은 `RLocalCachedMap`에서 local hit와 Redis hit를 따로 얻지 못합니다. 그래서 `localHits`, `localMisses`, `localEvictions`은 0이고, `backHits`와 `backMisses`는 wrapper의 통합 get 결과를 셉니다.
