...
description: "bluetape4k-cache-core는 캐시 기능의 공통 API, 핵심 추상화, 그리고 로컬 캐시 구현체를 제공하는 모듈입니다."
kind: library
group: caching
---
...
bluetape4k-cache-core는 캐시 기능의 공통 API, 핵심 추상화, 그리고 로컬 캐시 구현체를 제공하는 모듈입니다. 이 매뉴얼은 README의 기능 목록을 반복하지 않고 현재 build, source entry point, test, 설정 resource, lifecycle 근거를 연결합니다.
