...
block이나 commit이 실패하면 rollback을 시도합니다. 1.11.0에서 rollback 실패는 warn log로만 남고 원래 예외에 suppressed로 붙지 않습니다. 원래 예외는 유지되지만 rollback 원인을 사후 분석하려면 해당 logger를 수집해야 합니다.
