...
아래 장은 API 목록을 반복하지 않습니다. 1.11.0 배포 소스와 대표 테스트를 바탕으로 ownership, failure, 일관성 경계를 설명하고 바로 적용할 수 있는 예제와 확인할 source link를 함께 제공합니다.
...
3. [Codec과 보안](./bluetape4k-redisson/codecs-security-wire-format.md) — Fory·JSON·압축 Codec의 wire format, allow-list와 migration 위험을 정리합니다.
4. [Local Cached Map과 무효화](./bluetape4k-redisson/local-cache-pubsub-invalidation.md) — JVM front cache, Redis back cache, Pub/Sub 동기화와 reconnect 정책을 다룹니다.
5. [Cache mode와 DB persistence](./bluetape4k-redisson/cache-modes-persistence.md) — cache-aside와 진짜 read/write-through·write-behind를 loader/writer 경계로 구분합니다.
6. [Lifecycle·테스트·생태계 경로](./bluetape4k-redisson/lifecycle-testing-ecosystem.md) — 종료, 장애 관찰, Testcontainers, cache-redisson·Exposed·workshop으로 이어지는 선택을 설명합니다.
...
Spring Cache abstraction이 필요하면 [`bluetape4k-cache-redisson`](./bluetape4k-cache-redisson.md)을 사용합니다. DB entity를 Exposed repository와 함께 cache하려면 5·6장의 외부 repository 경로를 따릅니다.
...
1.11.0 대표 테스트는 실제 Redis를 Testcontainers로 시작해 client, Stream, cache, coroutine과 Codec 계약을 검증합니다.
...
이 매뉴얼은 `bluetape4k-projects` 1.11.0 배포 commit을 기준으로 합니다. `RedissonNearCache`는 `RLocalCachedMap`에 위임하며 별도의 두 map을 수동으로 일관되게 갱신하는 abstraction이 아닙니다. `destroy()`는 local near-cache instance만 정리하고 Redis data는 남깁니다.

`RedissonCacheConfig`의 preset 이름만으로 DB read/write-through가 생기지 않습니다. `MapLoader`와 `MapWriter` 구현을 options에 실제로 연결해야 하며, `deleteFromDBOnInvalidate`는 1.11.0 option 변환에서 지원하지 않습니다.
...
[![Codec 선택 맵 다이어그램](../../assets/readme-diagrams/infra-redisson-diagram-01.png)](../../assets/readme-diagrams/infra-redisson-diagram-01.svg)

_배포본 README: [`infra/redisson/README.ko.md`](https://github.com/bluetape4k/bluetape4k-projects/blob/6187173b58e8b4c5c435c145e00e94708f31ef75/infra/redisson/README.ko.md)_

### Batch / Transaction 다이어그램

[![Batch / Transaction 다이어그램](../../assets/readme-diagrams/infra-redisson-diagram-02.png)](../../assets/readme-diagrams/infra-redisson-diagram-02.svg)

_배포본 README: [`infra/redisson/README.ko.md`](https://github.com/bluetape4k/bluetape4k-projects/blob/6187173b58e8b4c5c435c145e00e94708f31ef75/infra/redisson/README.ko.md)_

### NearCache 2-Tier 다이어그램

[![NearCache 2-Tier 다이어그램](../../assets/readme-diagrams/infra-redisson-sequence-01.png)](../../assets/readme-diagrams/infra-redisson-sequence-01.svg)
