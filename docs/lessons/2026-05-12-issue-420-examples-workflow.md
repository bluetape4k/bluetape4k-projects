...
- `./gradlew -q projects | rg "bluetape4k-examples|idgenerator-ktor-demo|idgenerator-spring-boot-demo"`로 현재 branch의 기존 examples 4개 모듈만 확인.
...
- `./gradlew build -x test ... --dry-run | rg "bluetape4k-examples|idgenerator-ktor-demo|idgenerator-spring-boot-demo"` 결과 match 없음으로 Nightly build exclusion 확인.
