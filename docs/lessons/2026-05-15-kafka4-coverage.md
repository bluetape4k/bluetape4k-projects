# 배운 점 — kafka4 coverage (2026-05-15)
...
### 교훈
Kafka Streams kstream DSL test 작성 시 `StreamPartitioner` lambda는
...
### 교훈
`suspendCancellableCoroutine` + `execute { producer -> producer.send(...) { ... } }` pattern의
함수는 반드시 실제 Kafka broker(Testcontainers)를 이용한 integration test로 cover해야 한다.
...
### 교훈
`koverXmlReport` 생성 후 Python으로 XML을 parsing해 package/file별 uncovered line 수를
...
### 문제
`mcp__plugin_github_github__create_pull_request` tool이 private repo에서 404 error를 반환했다.

### 교훈
bluetape4k-projects는 private repo이므로 모든 GitHub 작업은 `gh` CLI를 사용한다.
