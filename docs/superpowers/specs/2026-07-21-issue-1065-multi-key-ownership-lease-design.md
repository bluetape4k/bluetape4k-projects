...
최악의 내부 Redis command 수는 acquire/inspect/release가 `2n`이고, renew는 owned key마다
`GET + PTTL + PEXPIRE`를 수행하므로 `3n`이다. 기본 `maxKeys = 32`는 한 번의 script에서
최대 96개 내부 command로 작업을 제한하는 보수적 상한이다.
