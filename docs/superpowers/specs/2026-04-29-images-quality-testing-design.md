...
- **속성 기반 검증 (Property-Based
  Testing)** 부재 — 이미지 변환의 수학적 불변식 (차원 보존, round-trip 무손실, pHash 견고성 등)을 광범위한 입력 공간에서 검증할 수 있는 메커니즘이 없다.
...
**bluetape4k-assertions 사용
규칙**: 비교는 `shouldBe` / `shouldBeEqualTo` / `shouldBeLessOrEqualTo` / `shouldBeLessThan` / `shouldBeGreaterOrEqualTo` 등 전용 matcher 를 사용한다. `(x == y).shouldBeTrue()` 류는 실패 시 값 맥락이 사라지므로 금지.

**실패
assertion**: 분기 안에서 명시적으로 실패가 필요할 때는 `org.junit.jupiter.api.Assertions.fail("...")` (AssertionError) 사용. `error()` (IllegalStateException) 금지.
...
- **Mac 로컬
  가능**: `brew install vips` 후 `DYLD_LIBRARY_PATH=/opt/homebrew/lib` (Apple Silicon) 또는 `/usr/local/lib` (Intel) 환경변수 설정 필요. Gradle test task 에 다음을 추가:
...
> **`BenchmarkImageSets.kt`
위치**: 비표준 sourceSet 인 `benchmark/` 가 아닌 표준 `main/` sourceSet 에 배치하여 IDE 인덱싱/공용 활용을 보장한다 (리뷰 [high] 반영).
...
- Zauner, C. (2010). *Implementation and Benchmarking of Perceptual Image Hash
  Functions*. Master's thesis. — pHash 64-bit 해밍 거리 기반 스케일 불변 임계값 (≤10) 근거.
