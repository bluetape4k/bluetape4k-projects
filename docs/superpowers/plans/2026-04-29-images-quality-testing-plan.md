...
본 플랜은 spec 의 9개 Phase 를 27개의 검증 가능한 Task 로 분해한다 (T4-A, T16-A 신설). 각 Task 는
...
| # | 결정 | 영향 |
|---|------|------|
| 1 | java21/java25 골든 이미지 **공유** — `images-vips-api` testFixtures 의 `golden/vips/` 경로에서 양쪽이 로드 | java21 별도 골든 생성 불필요. T11(java21) 은 T16(java25 골든 생성) 완료 이후 진행 |
...
| `.github/workflows/nightly-tests.yml` | `test-images` 신설 + `test-images-vips` 보강 + `images-benchmark` 매트릭스 |
...
| G6 (CI yaml) | **T21 → T22 → T23 직렬** | 단일 파일 `nightly-tests.yml` 동시 편집 시 충돌 → 직렬화 강제 |
| G7 (README) | T24 의 4개 모듈 README | 각 모듈 README.md/README.ko.md 동시 편집 |
...
- **설명**: spec §3.6.2 패턴을 따른다.
  ...
  - **`benchmarkImplementation` extendsFrom 정정**: spec 원본의 `testImplementation` 포함은 JUnit/MockK 등 테스트 프레임워크가 benchmark runtime 에 들어가는 부작용이 있으므로 제외한다. 최종:
    ```kotlin
    configurations {
...
- **완료 조건**:
  ...
  - `./gradlew :bluetape4k-images-benchmark:dependencies --configuration benchmarkRuntimeOnly` 출력에 `bluetape4k-images-vips-java25` (default) 또는 `-java21` (`-Pvips.impl=java21`) 가 포함, `junit`/`mockk` 미포함 확인.
...
- **설명**: java21/java25 양쪽이 공유할 vips 전용 골든 비교 유틸을 `images-vips-api` 의 testFixtures 에 배치한다.
  ...
  - **갱신 모드**: `System.getProperty("bluetape4k.images.golden.update") == "true"` 로 진입. **반드시 먼저 `ensureNotCi()` 호출** → CI 에서 `IllegalStateException`. 갱신 후 `org.opentest4j.TestAbortedException` throw 하여 skipped 처리.
...
- **설명**:
  - 각 클래스는 `AbstractImageTest` 상속 + `companion object: KLoggingChannel()`.
...
- **완료 조건**:
  - 갱신 모드 실행 (`./gradlew :bluetape4k-images:test --tests "io.bluetape4k.images.golden.*" -Dbluetape4k.images.golden.update=true`) 으로 T6 의 8장 골든이 `src/test/resources/golden/` 에 자동 생성 → T6 가 자동 충족.
...
- **complexity**: low
- **대상 파일**: 없음 (검증 단계)
- **설명**: 로컬에서 `-Dbluetape4k.images.golden.update=true` 실행 → skipped + 파일 갱신 확인. CI 환경 시뮬레이션 `CI=1 ... -Dbluetape4k.images.golden.update=true` → `IllegalStateException` 으로 빌드 실패 확인.
- **완료 조건**:
  - 로컬 갱신 → `TestAbortedException` (skipped) + 파일 갱신 확인.
...
- **설명**: spec §3.3.2 의 10개 불변식 구현. `@ParameterizedTest @MethodSource("io.bluetape4k.images.property.ImagePropertyInputs#resizeDimensions")` 등으로 입력 주입.
  ...
  - `AbstractImageTest` 상속, `companion object: KLoggingChannel()`.
- **완료 조건**:
...
- **설명**: T14 를 갱신 모드로 1회 실행하여 골든을 `images-vips-api/src/testFixtures/resources/golden/vips/` 에 자동 생성.
  - Mac 로컬 (`brew install vips`) 환경에서 실행: `./gradlew :bluetape4k-images-vips-java25:test -Dvips.enabled=true -Dbluetape4k.images.golden.update=true --tests "io.bluetape4k.images.vips.java25.golden.VipsFfmGoldenTest"`
  - 검증 모드 통과 확인 후 커밋.
- **완료 조건**:
...
- **설명**: spec §3.6.4 — `@Setup(Level.Trial)` 에서만 vips 초기화. `VipsInitializer.ensureInitialized()` 호출. `open class VipsBenchmarkState { @Setup(Level.Trial) fun init() { ... } }`. T18/T19 의 vips 포함 벤치마크 클래스가 `VipsBenchmarkState` 상속.
  - **멱등성 검증 (spy/log 의존 폐기)**: 단위 테스트 `@BeforeAll` 에서 `VipsInitializer.ensureInitialized()` 를 **연속 2회 호출 후 `assertDoesNotThrow`** 로 검증. setup 로그 출력 확인이 아닌 명시적 단위 테스트.
...
### Phase 8 — CI 연동 (`nightly-tests.yml`) — **직렬화 강제**

> **직렬화 사유**: T21/T22/T23 모두 단일 파일 `nightly-tests.yml` 을 편집하므로 동시 편집 시 머지 충돌 발생. 병렬 실행 가이드(G6) 에서 명시적으로 직렬 실행을 강제한다.

#### T21 — `nightly-tests.yml` `test-images` job 신설
- **complexity**: medium
- **대상 파일**: `.github/workflows/nightly-tests.yml`
- **설명**: spec §3.8.1 그대로. `needs: [build]`, `actions/setup-java@v4` (Temurin 21), `./gradlew :bluetape4k-images:test`, `if: failure()` 시 `actions/upload-artifact@v4` 로 `images/images/build/reports/golden-diffs/**` 업로드 (`retention-days: 7`).
  - ci.yml 에 동일한 step 이 이미 있는지 검토 — 있으면 nightly 와 동기화 (memory rule `feedback_ci_nightly_sync`). 없으면 ci.yml 에도 동일 잡 신설 검토 필요 (별도 PR 코멘트로 결정).
- **완료 조건**:
  - YAML 문법 검증 (`yq '.jobs."test-images"' .github/workflows/nightly-tests.yml`).
  - PR push 후 nightly trigger 시 잡이 표시 (또는 `workflow_dispatch` 로 수동 실행 가능).
- **선행 Task**: T7

#### T22 — `nightly-tests.yml` `test-images-vips` diff artifact 보강
- **complexity**: low
- **대상 파일**: `.github/workflows/nightly-tests.yml`
- **설명**: 기존 `test-images-vips` job 끝에 spec §3.8.2 의 step 추가:
  ```yaml
  - name: Upload vips golden diff reports
    if: failure()
    uses: actions/upload-artifact@v4
    with:
      name: golden-diffs-images-vips-${{ matrix.<KEY> }}
      path: |
        images/images-vips-java21/build/reports/golden-diffs/**
        images/images-vips-java25/build/reports/golden-diffs/**
      retention-days: 7
  ```
  - **matrix 키 이름 검증 선행**: 구현 전 `bat .github/workflows/nightly-tests.yml | rg "matrix"` 로 실제 키 이름(`java`, `java-version`, `vips-impl` 등) 확인. spec 의 `matrix.java` 가 실제와 다르면 실제 키로 치환.
- **완료 조건**:
  - `yq '.jobs."test-images-vips"' .github/workflows/nightly-tests.yml` 문법 통과.
  - **실제 yml matrix 키 이름과 일치 확인** (artifact name 의 `${{ matrix.<KEY> }}` 가 실재 키 참조).
- **선행 Task**: T12, T15, T21
...
- **설명**: spec §3.8.3 그대로.
  ...
  - **JMH fork 옵션**은 클래스 `@Fork(jvmArgs = [...])` 가 담당하므로 Gradle `-Dorg.gradle.jvmargs=...` 는 제거 (fork JVM 에 전달 안 됨).
...
- **설명**:
  ...
  - `images-vips-java25` README: Mac 로컬 실행 가이드 (`brew install vips`, `DYLD_LIBRARY_PATH`, `--enable-native-access=ALL-UNNAMED`) + **골든 마스터 환경** 명시.
...
3. **단계 3 (CI 통합 — 직렬 강제)**: T21 → T22 → T23 (단일 yml 파일 충돌 방지)
4. **단계 4 (문서)**: T24 (4개 모듈 README 동시 편집) → T25 (벤치마크 결과 + wiki)
...
| medium | 14개 (T1, T5, T7, T9, T11, T13, T14, T15, T16, T17, T20, T21, T23, T24, T25 일부) | 표준 sonnet worker |
| low | 5개 (T3, T6, T8, T12, T16-A, T22) | haiku worker |
...
- **CI yml 변경 시**: `feedback_ci_nightly_sync` — ci.yml 의 동기화 검토 (T21 에서 명시적으로 검토).
...
- **Kluent matcher**: `feedback_kluent_comparison_matchers` — `(x==y).shouldBeTrue()` 금지 (T10/T13/T16).
...
- **신규 CI job 2개 + 보강 1개**: `test-images` 신설, `images-benchmark` 매트릭스 신설, `test-images-vips` 보강

총 **27 Task**, 9 Phase (T4-A, T16-A 신설).
