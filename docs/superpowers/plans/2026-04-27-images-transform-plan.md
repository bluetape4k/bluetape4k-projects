...
- **what to implement** (spec §7.2):
    ...
    - **예외 로깅
      검증**: `perspectiveTransform` DSL op 에 의도적으로 예외를 발생시키는 잘못된 파라미터를 전달 → `log.warn` 이 호출됨을 logback `ListAppender` 를 사용하여 검증 — WARN 레벨 로그 메시지에 op 이름 (`perspectiveTransform`)이 포함되는지 확인.
