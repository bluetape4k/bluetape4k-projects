...
run-evidence.py record-rollback --state build/issue-757-evidence/jar.json --dispatch serializer_encode|serializer_decode|redisson_contiguous [--dispatch ...] --archive-root .omx/evidence/issue-757-rollback
run-evidence.py finalize-rollback --preparation .omx/evidence/issue-757-rollback/rollback-preparation-gN-<sha256>.json
...
If `origin/develop` is not an ancestor, stop delivery and merge the fetched base into the feature branch with a merge commit; do not rebase, amend, or squash authenticated evidence commits. Resolve conflicts, apply the evidence invalidation matrix to every affected surface, and rerun all of Task 9 plus any Task 8 lifecycle required by those changes. A merge commit preserves an authenticated commit graph only for commits that were already ancestors of the merged branch; it cannot restore measurement or rollback commits lost by an earlier history rewrite. After the historical rebase and audited tree-equivalent `rebind-rebased-delivery` recovery above, only delivery ancestry is re-established, while measurement/rollback SHAs remain immutable manifest/hash-chain identities without an asserted Git-reachability guarantee. If executable full ancestry for those identities is required and no explicit retained anchor exists, collect fresh evidence. Rebase or squash is prohibited for an evidence-bound PR because it rewrites the ancestry that still exists. Do not push stale-base evidence.
