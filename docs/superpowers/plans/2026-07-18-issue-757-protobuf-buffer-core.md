...
If `origin/develop` is not an ancestor, stop delivery and merge the fetched base into the feature branch with a merge commit; do not rebase, amend, or squash authenticated evidence commits. Resolve conflicts, apply the evidence invalidation matrix to every affected surface, and rerun all of Task 9 plus any Task 8 lifecycle required by those changes. The merge commit preserves the authenticated measurement, rollback, and delivery commits as ancestors; rebase or squash rewrites that lineage and is prohibited for an evidence-bound PR. If an earlier rebase already happened, use the audited tree-equivalent `rebind-rebased-delivery` recovery above or collect fresh evidence when its preconditions fail. Do not push stale-base evidence.
