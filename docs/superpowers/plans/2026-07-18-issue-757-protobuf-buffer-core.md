...
`record-rollback` is permitted only when the state-bound comparison has a mapped `regressed` verdict. One invocation accepts every simultaneously regressed dispatch as repeatable `--dispatch` values, sorts them by the fixed order `serializer_encode`, `serializer_decode`, `redisson_contiguous`, and records them atomically or records none. It copies the complete two-run inputs, comparison/validation, metadata, argv/logs, and current state into one unique immutable archive. It then creates, with fail-if-exists semantics under `--archive-root`, a canonical `rollback-bundle-g<generation>-<payload-sha256>.json`, stores/prints its absolute path, and includes the imported predecessor-bundle hash if any plus an ordered `decisions` array; every decision contains old commit/tree, dispatch unit, exact mapped regressed cells, archive-relative paths and SHA-256 values, timestamp, generation number, and a SHA-256 over its decision payload. A chained invocation appends new decisions to a new immutable bundle only after authenticating the imported bundle and requires the current source lineage to descend from the preceding post-rollback head; it never edits or overwrites an older bundle. Duplicate or conflicting dispatch decisions, a partial simultaneous-regression set, missing generations, and two decisions for the same dispatch fail. Only after the archive and bundle are durable does the command mark the old state non-promotable.

A subsequent fresh `resolve-jar --rollback-bundle <absolute bundle path>` imports the complete bundle hash and authenticated ordered decision/archive set into the new state. `compare`, `promote`, `verify-promoted`, the delivery manifest, and delivery loaders must carry and verify that exact set. After each named dispatch is removed, metadata/fixture tests mark all of its mapped candidate cells `ineligible` with reason `removed_after_regression`; the final two runs validate correctness/compatibility but cannot create a positive claim for removed units. Retained units remain eligible and a later regression creates the next bundle generation before another fresh run. Final promotion includes every immutable rollback archive, bundle generation, and the two fresh final runs. Fixtures cover two and three simultaneous regressions, a later chained regression, duplicate/conflicting dispatches, partial bundle writes, missing/reordered generations, altered archives, missing old runs, wrong dispatch, and bundle reuse across a different source lineage.
...
- `regressed`: both runs are at least `+5%`; collect every simultaneously regressed mapped unit and run one atomic `record-rollback` before any code change, apply the rollback matrix below, recommit, and restart Task 8 from a new clean exact head importing the complete bundle.
...
Before a rollback, execute one `record-rollback` with every mapped regressed dispatch and verify the resulting bundle/decision/archive hashes. Move the now non-promotable working staging root to a unique `benchmark/protobuf-codec-benchmark/build/issue-757-evidence-obsolete-<UTC>-<old-head>/`; immutable rollback archives remain separately referenced by the bundle. Commit the symbol-scoped code/docs changes with a Lore message, run the affected targeted tests plus `:bluetape4k-protobuf:test`, both Python fixture suites, benchmark test/compile/jar, and the final clean/scope gates from Task 9. Build a fresh JAR, resolve a fresh state with `--rollback-bundle`, and perform two entirely new run IDs. Old metrics may justify only the removal decisions; they cannot be reused as final-head performance evidence. The final comparison renders every removed unit's mapped cells `ineligible` with reason `removed_after_regression`, while retained units still obey accepted/inconclusive/regressed; any later retained-unit regression records a new authenticated bundle generation and restarts this paragraph.
