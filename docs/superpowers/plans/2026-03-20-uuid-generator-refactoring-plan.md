...
**날짜**: 2026-03-20 **Spec 참조**: `docs/superpowers/specs/2026-03-20-uuid-generator-refactoring-design.md`
...
`utils/idgenerators` 모듈의 UUID 생성기가 구조적으로 불일치하고 중복이 있음. ULID 통합에서 확립된 패턴 (`object` 진입점 + nested interface/objects + 어댑터 class)을 UUID에도 적용하여 통일된 API를 제공한다.
...
**complexity**: medium **의존성**: 없음 **변경
파일**: 신규 `utils/idgenerators/src/main/kotlin/io/bluetape4k/idgenerators/uuid/Uuid.kt`
...
**complexity**: low **의존성**: Task 1 **변경
파일**: 신규 `utils/idgenerators/src/main/kotlin/io/bluetape4k/idgenerators/uuid/UuidGenerator.kt`
...
**complexity**: medium **의존성**: Task 1 **변경 파일**:
...
**complexity**: medium **의존성**: Task 1, Task 2 **변경 파일**:
...
**complexity**: low **의존성**: Task 1, 2, 3, 4 **변경 파일**: 없음 (검증만)
