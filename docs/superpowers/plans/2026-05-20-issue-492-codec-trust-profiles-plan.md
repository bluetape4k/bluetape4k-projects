...
2. Harden `RedissonProtobufCodec`.
    ...
    - Rethrow `SecurityException`; do not silently fall back after a trust boundary violation.
...
4. Add representative tests.
    ...
    - Explicit unsafe opt-in attempts legacy class loading; for a missing class it falls through to the existing fallback path instead of throwing the allowlist `SecurityException`.
...
5. Update public docs.
    ...
    - Update `io/protobuf` README pair for protobuf and Redisson protobuf defaults.
    - Update `infra/redisson` README pair for trusted-internal vs allowlisted codecs.
    - Update Kafka and Kafka4 README pairs with secure and unsafe legacy examples; fix Kafka4 `emptySet()` drift.
...
7. Run current-session Step 6-R review, write lesson, commit, push, create PR, add PR review comment, and wait for CI.
