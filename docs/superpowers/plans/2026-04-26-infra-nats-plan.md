...
- **참고**: `io/bluetape4k/nats/SimplePublishExample.kt`는 이미 `io.bluetape4k.nats` 패키지를 사용하므로 **리네이밍 대상
  아님** — `git mv infra/nats` 시 자동 이동만 수행
- **action**:
    - `git mv` 로 순서대로 이동 (하위 → 상위 순):
        ...
        5. **별도
           step**: `src/test/kotlin/io/nats/examples/service/ServiceExample.kt` (1개) → `src/test/kotlin/io/bluetape4k/nats/service/examples/ServiceExample.kt`
...
- **옵션
  1**: `testing/testcontainers/src/main/kotlin/io/bluetape4k/testcontainers/mq/NatsServer.kt` tag 2.12 → 최신 버전 (별도 commit, 본 PR 포함)
