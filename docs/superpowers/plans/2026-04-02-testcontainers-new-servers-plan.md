...
**날짜**: 2026-04-02 **대상 모듈**: `testing/testcontainers` (`bluetape4k-testcontainers`)
...
- **작업 내용**:
    ...
    - **`invoke` 팩토리에 `image.requireNotBlank("image")` / `tag.requireNotBlank("tag")` 호출 필수**
    - **AGE 초기화 SQL 실패
      시**: 예외를 그대로 전파하여 컨테이너 시작 실패로 처리. 단, 각 SQL 단계 전에 `log.info { "AGE 초기화: $sql 실행 중..." }` 로깅을 남겨 어떤 단계에서 실패했는지 추적 가능하게 할 것
- **acceptance criteria**:
