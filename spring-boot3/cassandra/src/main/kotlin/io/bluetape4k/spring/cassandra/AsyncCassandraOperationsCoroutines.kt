 ...
 *
 * ```kotlin
 * val resultSet = asyncOps.executeSuspending(statement)
 ...
 * - `select<T>(statement).await() ?: emptyList()`를 호출합니다.
 *
 * ```kotlin
 ...
 *
 * ## 동작/계약
 * - `select<T>(statement) { consumer(it) }.await()`를 호출합니다.
 * - 결과가 없으면 [consumer]는 호출되지 않습니다.
 *
 * ```kotlin
 * asyncOps.selectSuspending<User>(statement) { user -> println(user.name) }
 * ```
 */
 ...
 *
 * ## 동작/계약
 * - CQL 문자열을 Statement로 변환한 뒤 [selectSuspending]에 위임합니다.
 * - 결과가 없으면 빈 리스트를 반환합니다.
 *
 * ```kotlin
 ...
 *
 * ```kotlin
 * asyncOps.selectSuspending<User>("SELECT * FROM users") { user -> println(user.name) }
 * ```
 */
suspend inline fun <reified T: Any> AsyncCassandraOperations.selectSuspending(
    cql: String,
 ...
 *
 * ## 동작/계약
 * - `select<T>(query, consumer).await()`를 호출합니다.
 * - 결과가 없으면 [consumer]는 호출되지 않습니다.
 *
 * ```kotlin
 * asyncOps.selectSuspending<User>(query) { user -> println(user.name) }
 * ```
 */
suspend inline fun <reified T: Any> AsyncCassandraOperations.selectSuspending(
    query: Query,
 ...
 *
 * ## 동작/계약
 * - `selectOne<T>(statement).await()`를 호출합니다.
 ...
 *
 * ## 동작/계약
 * - CQL 문자열을 Statement로 변환한 뒤 [selectOneOrNullSuspending]에 위임합니다.
 *
 * ```kotlin
 * val user = asyncOps.selectOneOrNullSuspending<User>("SELECT * FROM users WHERE id = 1")
 * // user == null || user is User
 * ```
 */
 ...
 *
 * ## 동작/계약
 * - `select<T>(query).await() ?: emptyList()`를 호출합니다.
 * - 결과가 없으면 빈 리스트를 반환합니다.
 *
 * ```kotlin
 * val users = asyncOps.selectSuspending<User>(query)
 * // users.size >= 0
 * ```
 */
 ...
 *
 * ## 동작/계약
 * - `selectOne<T>(query).await()`를 호출합니다.
 * - 결과가 없으면 `null`을 반환합니다.
 *
 * ```kotlin
 * val user = asyncOps.selectOneOrNullSuspending<User>(query)
 * // user == null || user is User
 * ```
 */
 ...
 *
 * ```kotlin
 * val slice = asyncOps.sliceSuspending<User>(query)
 * // slice.content.size >= 0
 * ```
 */
 ...
 *
 * ## 동작/계약
 * - `update<T>(query, update).await()`를 호출합니다.
 * - 조건에 맞는 행이 없으면 `null`이 반환될 수 있습니다.
 *
 * ```kotlin
 ...
 *
 * ## 동작/계약
 * - `delete<T>(query).await()`를 호출합니다.
 * - 조건에 맞는 행이 없으면 `null`이 반환될 수 있습니다.
 *
 * ```kotlin
 * val deleted = asyncOps.deleteSuspending<User>(query)
 ...
 *
 * ## 동작/계약
 * - `count<T>().await()`를 호출합니다.
 * - 결과가 없으면 `null`이 반환될 수 있습니다.
 *
 * ```kotlin
 ...
 *
 * ## 동작/계약
 * - `count<T>(query).await()`를 호출합니다.
 * - 결과가 없으면 `null`이 반환될 수 있습니다.
 *
 * ```kotlin
 * val count = asyncOps.countSuspending<User>(query)
 * // count != null && count >= 0
 * ```
 */
 ...
 *
 * ## 동작/계약
 * - `exists<T>(query).await()`를 호출합니다.
 *
 * ```kotlin
 * val exists = asyncOps.existsSuspending<User>(query)
 * // exists == true || exists == false
 * ```
 */
 ...
 *
 * ## 동작/계약
 * - `selectOneById<T>(id).await()`를 호출합니다.
 * - 결과가 없으면 `null`을 반환합니다.
 *
 * ```kotlin
 ...
 *
 * ## 동작/계약
 * - `insert(entity, options).await()`를 호출합니다.
 * - LWT 결과 등은 [EntityWriteResult]에서 확인합니다.
 *
 * ```kotlin
 ...
 *
 * ## 동작/계약
 * - `update(entity, options).await()`를 호출합니다.
 * - LWT 결과 등은 [EntityWriteResult]에서 확인합니다.
 *
 * ```kotlin
 * val result = asyncOps.updateSuspending(user, updateOptions { timeout(500) })
 * // result.wasApplied() == true
 * ```
 */
 ...
 *
 * ## 동작/계약
 * - `deleteById<T>(id).await()`를 호출합니다.
 * - 삭제 성공 여부를 Boolean으로 반환합니다.
 *
 * ```kotlin
 ...
 *
 * ## 동작/계약
 * - `delete(entity, options).await()`를 호출합니다.
 *
 * ```kotlin
