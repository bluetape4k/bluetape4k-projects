...
class SystemPropertyExtensionMetaTest {
    ...
    fun `클래스 단위로 시스템 속성은 원복된다`() {
        ...
        // 복원된 속성
        System.getProperty("classPropertyC") shouldBeEqualTo "no"
    }
    ...
    fun `클래스 단위로 시스템 속성들이 원복된다`() {
        ...
        // 복원된 속성
        System.getProperty("classPropertyA") shouldBeEqualTo "no"
        System.getProperty("classPropertyB") shouldBeEqualTo "yes"
    }
    ...
    fun `메소드 단위로 시스템 속성은 원복된다`() {
        ...
        //테스트 수행 시에는 classPropertyValueC 로 설정된다
        ExtensionTester.execute(
            selectMethod(
                SystemPropertyExtensionClassTest::class.java,
                "메소드 단위로 테스트를 위한 시스템 속성을 설정"
            )
        )

        // 복원된 속성
        System.getProperty("keyC") shouldBeEqualTo "no"
    }
