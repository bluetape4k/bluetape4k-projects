...
class SuspendedJobTesterTest {
    ...
    @Test
    fun `복수 라운드에서 발생한 예외는 MultiException으로 수집된다`() =
        runTest {
    ...
    fun `workers 범위 초과값 2001 을 설정하면 예외가 발생한다`() {
        assertFailsWith<IllegalArgumentException> {
            SuspendedJobTester().workers(2001)
        }
    }
