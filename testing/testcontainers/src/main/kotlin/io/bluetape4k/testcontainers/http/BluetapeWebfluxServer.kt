...
/**
 * `bluetape4k/mock-webflux-server` Docker 이미지를 로컬 컨테이너로 실행하는 테스트 HTTP 서버입니다.
 *
 * Spring Boot 4 WebFlux + Coroutines 기반 mock HTTP 서버로, 포트 9999에서
 * httpbin/jsonplaceholder/web 엔드포인트를 제공합니다.
 *
 * ## 동작/계약
 * - `/ping` 엔드포인트가 HTTP 200을 반환할 때까지(최대 60초) 시작을 기다립니다.
 * - `useDefaultPort=true`이면 [PORT](9999) 포트를 호스트에 고정 바인딩하려고 시도하고,
 *   그렇지 않으면 동적 포트를 사용합니다.
 * - `reuse=true`일 때 Testcontainers 재사용 옵션을 켭니다.
 * - 인스턴스 생성만으로는 컨테이너가 시작되지 않으며, `start()` 호출이 필요합니다.
 *
 * ```kotlin
 * val server = BluetapeWebfluxServer()
 * server.start()
 * val pingUrl = "${server.url}/ping"
 * // pingUrl == "http://${server.host}:${server.port}/ping"
 * ```
 *
 * Docker 이미지: `bluetape4k/mock-webflux-server`
 */
class BluetapeWebfluxServer private constructor(
    ...
    companion object: KLogging() {
         ...
         * @param useDefaultPort `true`면 9999 포트를 고정 바인딩합니다.
        ...
        /**
         * 이미지 이름과 태그로 [BluetapeWebfluxServer] 인스턴스를 생성합니다.
         *
         * ## 동작/계약
     ...
     * // props["httpbin-url"] == "http://localhost:9999/httpbin"
     * ```
     */
     ...
     *   `testcontainers.bluetape-webflux.host`, `testcontainers.bluetape-webflux.port`,
     *   `testcontainers.bluetape-webflux.url` 프로퍼티를 시스템에 등록합니다.
     *
     * ```kotlin
     * server.start()
     * // System.getProperty("testcontainers.bluetape-webflux.url") != null
     * ```
     */
    override fun start() {
        super.start()
    ...
    /**
     * 테스트 전역에서 재사용할 [BluetapeWebfluxServer] 싱글턴을 제공합니다.
     *
     * ## 동작/계약
     * - [bluetapeWebfluxServer]에 처음 접근할 때 서버를 시작하고 [ShutdownQueue]에 종료 훅을 등록합니다.
     ...
     */
    object Launcher {
        /** 지연 초기화되는 재사용용 Bluetape WebFlux 서버입니다. */
