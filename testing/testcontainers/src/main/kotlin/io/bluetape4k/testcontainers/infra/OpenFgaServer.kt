...
/**
 * 공식 [OpenFGAContainer]의 준비 상태 검사를 사용하는 테스트 서버입니다.
 *
 ...
 */
class OpenFgaServer private constructor(
    imageName: DockerImageName,
    useDefaultPort: Boolean,
    reuse: Boolean,
): OpenFGAContainer(imageName), GenericServer, PropertyExportingServer {

    companion object: KLogging() {
        /** 기본 Docker 이미지입니다. */
        const val IMAGE = "openfga/openfga"

        /** 검증에 사용하는 고정 이미지 태그입니다. */
        const val TAG = "v1.8.2"

        /** 시스템 속성 namespace입니다. */
        const val NAME = "openfga"
        ...
        operator fun invoke(
            ...
            return OpenFgaServer(DockerImageName.parse(image).withTag(tag), useDefaultPort, reuse)
        }
    }
    ...
    override fun start() {
        ...
        log.debug { "OpenFgaServer 시작: url=$url, grpcPort=$grpcPort" }
    }

    /** JVM 안에서 공유하며 [ShutdownQueue]가 종료하는 서버입니다. */
    object Launcher {
        /** 호출자가 종료하지 않는 공유 서버 인스턴스입니다. */
        val openFga: OpenFgaServer by lazy {
            OpenFgaServer().apply {
                start()
