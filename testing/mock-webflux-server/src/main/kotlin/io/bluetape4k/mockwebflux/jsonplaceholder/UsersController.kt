...
/**
 * jsonplaceholder `/jsonplaceholder/users` CRUD 엔드포인트 (WebFlux suspend).
 *
...
class UsersController(private val service: JsonplaceholderService) {
     ...
     */
    @GetMapping
    suspend fun list(): List<UserRecord> = service.users.all()
     ...
     */
    @GetMapping("/{id}")
    suspend fun getById(@PathVariable id: Long): UserRecord =
    ...
    suspend fun create(@RequestBody user: UserRecord): UserRecord = service.users.add(user)
     ...
     */
    @PutMapping("/{id}")
    suspend fun update(@PathVariable id: Long, @RequestBody user: UserRecord): UserRecord =
     ...
     */
    @PatchMapping("/{id}")
    suspend fun patch(@PathVariable id: Long, @RequestBody user: UserRecord): UserRecord =
        service.users.update(id, user) ?: throw NoSuchElementException("User $id not found")
