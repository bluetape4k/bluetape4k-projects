...
/**
 * httpbin 이미지 로더 서비스 (WebFlux + Coroutines 버전).
 *
 * 요청된 형식의 placeholder 이미지를 생성하고 캐시한다.
 * self-injection 없이 `@Cacheable`을 적용하기 위해 별도 Bean으로 분리한다.
 *
 * 이미지 인코딩은 CPU + blocking I/O (ImageIO)이므로 [Dispatchers.IO]에서 수행한다.
 */
@Service
class ImageLoaderService {
     ...
     *
     * `@Cacheable`은 suspend 함수와 직접 연동되지 않으므로,
     * 메서드 내부에서 `withContext(Dispatchers.IO)`로 블로킹 I/O를 감쌌다.
     *
     * @param fmt 이미지 형식 (png, jpeg, webp, svg)
     * @return 이미지 바이트와 Content-Type을 포함한 [ResponseEntity]
     */
    @Cacheable("httpbin-image", key = "#fmt")
    suspend fun loadImage(fmt: String): ResponseEntity<ByteArray> {
...
<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 100 100">
  ...
  <text x="50" y="55" text-anchor="middle" font-size="12" fill="#666666">bluetape4k</text>
                ...
                "png", "jpeg", "webp" -> {
                    ...
                    "Unsupported image format: $fmt. Allowed: $ALLOWED_FORMATS"
                )
            }
        }
    }
