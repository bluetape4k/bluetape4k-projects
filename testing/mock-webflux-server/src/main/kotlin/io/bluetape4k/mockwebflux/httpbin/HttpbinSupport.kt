package io.bluetape4k.mockwebflux.httpbin

import io.bluetape4k.mockwebflux.httpbin.model.HttpbinResponse
import org.springframework.http.server.reactive.ServerHttpRequest
import tools.jackson.core.type.TypeReference
import tools.jackson.databind.json.JsonMapper

private val jsonMapper = JsonMapper.builder().build()
private val mapTypeRef = object: TypeReference<Map<String, Any>>() {}

/**
 * WebFlux [ServerHttpRequest]의 헤더를 단일값 Map으로 변환한다.
 *
 * httpbin은 헤더를 단일 문자열로 직렬화하므로 다중값 헤더는 첫 번째 값만 사용한다.
 *
 * @return 헤더 이름과 첫 번째 값의 Map
 */
fun ServerHttpRequest.toHeaderMap(): Map<String, String> =
    headers.toSingleValueMap()

/**
 * [ServerHttpRequest]와 요청 바디를 [HttpbinResponse]로 변환한다.
 *
 * JSON Content-Type이면 body를 파싱해 `json` 필드에 담고,
 * form-urlencoded이면 `form` 필드에, 그 외는 `data` 필드에 담는다.
 *
 * @param body 요청 바디 문자열 (nullable)
 * @param method HTTP 메서드명
 * @return httpbin 형식의 응답 객체
 */
fun ServerHttpRequest.toHttpbinResponse(body: String? = null, method: String? = null): HttpbinResponse {
    val args: Map<String, String> = uri.rawQuery?.split("&")
        ?.filter { it.contains("=") }
        ?.associate { it.substringBefore("=") to it.substringAfter("=") }
        ?: emptyMap()

    val headerMap = toHeaderMap()
    val contentType = headers.getFirst("Content-Type") ?: ""

    val isJson = contentType.contains("application/json", ignoreCase = true)
    val isForm = contentType.contains("application/x-www-form-urlencoded", ignoreCase = true)

    val formData: Map<String, String> = if (isForm && body != null) {
        body.split("&")
            .filter { it.contains("=") }
            .associate { it.substringBefore("=") to it.substringAfter("=") }
    } else emptyMap()

    val jsonData: Map<String, Any>? = if (isJson && !body.isNullOrBlank()) {
        runCatching { jsonMapper.readValue(body, mapTypeRef) }.getOrNull()
    } else null

    // remoteAddress 는 InetSocketAddress; address 가 null 이면 hostString 으로 폴백
    val origin = remoteAddress?.let { it.address?.hostAddress ?: it.hostString } ?: ""
    val url = uri.toString()

    return HttpbinResponse(
        args = args,
        headers = headerMap,
        origin = origin,
        url = url,
        data = if (formData.isEmpty() && jsonData == null) body ?: "" else "",
        form = formData,
        json = jsonData,
        method = method ?: this.method.name(),
    )
}
