...
/**
 * Caffeine Cache를 이용하는 [AsyncCaffeineMemoizer]를 생성합니다.
 *
...
): AsyncCaffeineMemoizer<T, R> {
    return AsyncCaffeineMemoizer(this, evaluator)
}

fun <T: Any, R: Any> ((T) -> CompletableFuture<R>).withAsyncMemoizer(
    cache: Cache<T, R>,
): AsyncCaffeineMemoizer<T, R> {
    return AsyncCaffeineMemoizer(cache, this)
}
 ...
 */
class AsyncCaffeineMemoizer<T: Any, R: Any>(
