...
/**
 * InMemory를 이용하여 [SuspendInMemoryMemoizer]를 생성합니다.
 */
fun <T: Any, R: Any> (suspend (T) -> R).suspendMemoizer(): SuspendInMemoryMemoizer<T, R> =
    SuspendInMemoryMemoizer(this)

/**
 * 로컬 메모리에 suspend evaluator 실행 결과를 저장합니다.
 */
class SuspendInMemoryMemoizer<in T: Any, out R: Any>(
