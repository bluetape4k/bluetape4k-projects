...
fun <T: Any, R: Any> org.cache2k.Cache<T, R>.suspendMemoizer(
    ...
    SuspendCache2kMemoizer(this, evaluator)
...
fun <T: Any, R: Any> (suspend (T) -> R).withSuspendMemoizer(
    ...
    SuspendCache2kMemoizer(cache, this)
 ...
 * ```
 */
class SuspendCache2kMemoizer<in T: Any, out R: Any>(
    ...
    override suspend fun invoke(input: T): R {
        return cache.get(input) ?: run {
            ...
            this@SuspendCache2kMemoizer.cache.put(input, result)
            result
        }
    }
