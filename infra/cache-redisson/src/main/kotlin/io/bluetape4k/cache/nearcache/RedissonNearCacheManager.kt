 ...
 * ```
 */
class RedissonNearCacheManager(
...
//    private var closed: Boolean = false
    ...
    override fun <K: Any, V: Any, C: Configuration<K, V>> createCache(
        ...
        val backCache = with(jcacheManager<JCachingProvider>()) {
            getCache(cacheName)
                ?: run {
                    val redissonCfg = RedissonConfiguration.fromInstance(cacheRedisson, redissonConfiguration.jcacheConfig)
