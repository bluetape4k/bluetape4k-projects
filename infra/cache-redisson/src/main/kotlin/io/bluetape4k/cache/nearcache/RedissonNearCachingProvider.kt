 ...
 *
 * ## 동작/계약
 * - `(ClassLoader, URI)` 조합으로 [RedissonNearCacheManager]를 생성/재사용합니다.
 * - `close` 계열 호출은 등록된 매니저를 닫고 내부 맵에서 제거합니다.
 * - optional feature는 지원하지 않아 항상 `false`를 반환합니다.
 *
 * ```kotlin
 * val provider = RedissonNearCachingProvider()
 * val manager = provider.cacheManager
 * // manager != null
 * ```
 */
class RedissonNearCachingProvider: CachingProvider {
    ...
    private val managers = ConcurrentHashMap<ClassLoader, MutableMap<URI, RedissonNearCacheManager>>()
    ...
    override fun getCacheManager(uri: URI?, classLoader: ClassLoader?, properties: Properties?): CacheManager? {
        ...
        manager = RedissonNearCacheManager(redisson, cacheClassLoader, this, properties, cacheUri)
    ...
    override fun close(classLoader: ClassLoader) {
        log.info { "Close RedissonNearCachingProvider. classLoader=$classLoader" }
    ...
    override fun close(uri: URI, classLoader: ClassLoader) {
        log.info { "Close RedissonNearCachingProvider. uri=$uri, classLoader=$classLoader" }
