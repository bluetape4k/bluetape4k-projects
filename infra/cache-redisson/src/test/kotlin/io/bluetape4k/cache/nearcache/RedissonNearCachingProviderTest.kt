...
class RedissonNearCachingProviderTest {
    ...
    fun `CachingProvider 를 이용하여 NearCache 생성하기`() {
        val provider = Caching.getCachingProvider(RedissonNearCachingProvider::class.qualifiedName)
        provider.shouldNotBeNull()
        val manager = provider.cacheManager
        manager.shouldNotBeNull()
        manager shouldBeInstanceOf RedissonNearCacheManager::class
    ...
    fun `CachingProvider close 하기`() {
        val provider = jcachingProvider<RedissonNearCachingProvider>()
        provider.shouldNotBeNull()
        val manager = provider.cacheManager
        manager.shouldNotBeNull()
        manager shouldBeInstanceOf RedissonNearCacheManager::class
