...
| `RedisMap<V>`                | Generic 분산 Hash Map (sync + async). 코루틴 버전: `RedisSuspendMap<V>`      |
| `RedisSuspendMap<V>`         | Generic 분산 Hash Map (suspend 전용). `LettuceBinaryCodec<V>` 지원           |
| `RedisAtomicLong`            | 분산 AtomicLong (sync + async). 코루틴 버전: `RedisSuspendAtomicLong`        |
| `RedisSuspendAtomicLong`     | 분산 AtomicLong (suspend 전용)                                            |
| `RedisSemaphore`             | 분산 세마포어 (sync + async). 코루틴 버전: `RedisSuspendSemaphore`               |
| `RedisSuspendSemaphore`      | 분산 세마포어 (suspend 전용)                                                  |
| `RedisLock`                  | 분산 뮤텍스 락 (sync + async). 코루틴 버전: `RedisSuspendLock`                   |
| `RedisSuspendLock`           | 분산 뮤텍스 락 (suspend 전용)                                                 |
...
### RedisMap\<V\> — Generic 분산 Hash Map

```kotlin
import io.bluetape4k.redis.lettuce.codec.LettuceBinaryCodecs
import io.bluetape4k.redis.lettuce.map.RedisMap
import io.bluetape4k.redis.lettuce.map.RedisSuspendMap
...
// 동기/비동기
val map = RedisMap<Product>(connection, "products")
...
// 코루틴 전용
val suspendMap = RedisSuspendMap<Product>(connection, "products")
val p = suspendMap.get("p1")                       // suspend fun
suspendMap.put("p2", Product(2L, "Gadget"))
```

> **String 기본값 이유**: Lettuce 기본 코덱은 `StringCodec.UTF8`입니다.
> `RedisMap<V>`처럼 단순 저장/조회(HGET/HSET)는 바이너리 코덱 사용이 가능하지만,
> `RedisAtomicLong`/`RedisSemaphore`는 Redis의 `INCR`/`DECR` 명령이 10진수 문자열을 요구하므로
> `StatefulRedisConnection<String, String>`만 사용해야 합니다.

### RedisAtomicLong — 분산 AtomicLong

```kotlin
import io.bluetape4k.redis.lettuce.atomic.RedisAtomicLong
import io.bluetape4k.redis.lettuce.atomic.RedisSuspendAtomicLong
...
// 코루틴 전용
val suspendCounter = RedisSuspendAtomicLong(connection, "my-counter")
suspendCounter.incrementAndGet()
suspendCounter.addAndGet(5L)
```

### RedisSemaphore — 분산 세마포어

```kotlin
import io.bluetape4k.redis.lettuce.semaphore.RedisSemaphore
import io.bluetape4k.redis.lettuce.semaphore.RedisSuspendSemaphore
...
// 코루틴 전용
val suspendSemaphore = RedisSuspendSemaphore(connection, "my-semaphore", totalPermits = 3)
...
import io.bluetape4k.redis.lettuce.lock.RedisSuspendLock
...
// 코루틴 전용
val suspendLock = RedisSuspendLock(connection, "my-lock")
