...
class LettuceCountDownLatchTest: AbstractLettuceTest() {
    ...
    fun `zero count and stale generation preserve lifecycle`() = runSuspendIO {
        ...
        val name = "latch-boundaries-${randomName().substringAfterLast(':')}"
        ...
        try {
            ...
            latch.trySetCountAsync(3, LatchRequestId.random())
                .get(5.seconds) shouldBeEqualTo LatchSetCountResult.InvalidCount

            val completed = latch.trySetCount(0, LatchRequestId.from("zero-count"))
                .shouldBeInstanceOf<LatchSetCountResult.Created>()
            latch.getCount(completed.generation).shouldBeInstanceOf<LatchCountResult.Completed>()
            ...
            ).get(5.seconds) shouldBeEqualTo LatchAwaitResult.TimedOut
            suspending.await(
                active.generation,
            ...
            ).get(5.seconds) shouldBeEqualTo LatchAwaitResult.StaleGeneration
            ...
            latch.getCountAsync(active.generation).get(5.seconds) shouldBeEqualTo LatchCountResult.Closed
    ...
    fun `generation increases across delete and recreate while count stays at zero`() {
        val connection = LettuceTestUtils.client.connect(StringCodec.UTF8)
        val name = "latch-${randomName().substringAfterLast(':')}"
        ...
        try {
            ...
            val second = latch.trySetCount(1, LatchRequestId.from("set-2"))
                .shouldBeInstanceOf<LatchSetCountResult.Created>()
            second.generation.value shouldBeGreaterThan first.generation.value
        } finally {
            latch.close()
            connection.sync().del(*keys.all.toTypedArray())
            connection.close()
    ...
    fun `generation bound results preserve completed stale and not found states`() {
        val connection = LettuceTestUtils.client.connect(StringCodec.UTF8)
        val name = "latch-contract-${randomName().substringAfterLast(':')}"
        val keys = deriveLatchKeys(name, LatchConfig(), StringCodec.UTF8)
        connection.sync().del(*keys.all.toTypedArray())
        val latch = LettuceCountDownLatch.create(connection, name)
        ...
        try {
            ...
            val created = latch.trySetCount(1, request)
                .shouldBeInstanceOf<LatchSetCountResult.Created>()
    ...
    fun `waiter cap fails closed and async completion waits for durable cleanup`() {
        ...
        val connection = LettuceTestUtils.client.connect(StringCodec.UTF8)
        val name = "latch-waiters-${randomName().substringAfterLast(':')}"
        val keys = deriveLatchKeys(name, config, StringCodec.UTF8)
        connection.sync().del(*keys.all.toTypedArray())
        val latch = LettuceCountDownLatch.create(connection, name, config)
    ...
    fun `delete removes an expired waiter left by a failed client cleanup`() {
        ...
        val connection = LettuceTestUtils.client.connect(StringCodec.UTF8)
        val name = "latch-stale-waiter-${randomName().substringAfterLast(':')}"
        val keys = deriveLatchKeys(name, config, StringCodec.UTF8)
        connection.sync().del(*keys.all.toTypedArray())
        val latch = LettuceCountDownLatch.create(connection, name, config)
    ...
    fun `late cleanup from an old generation cannot remove a current waiter`() {
        val connection = LettuceTestUtils.client.connect(StringCodec.UTF8)
        val name = "latch-generation-waiter-${randomName().substringAfterLast(':')}"
    ...
    fun `async and suspend modes preserve the generation result matrix`() = runTest {
        val connection = LettuceTestUtils.client.connect(StringCodec.UTF8)
        val name = "latch-modes-${randomName().substringAfterLast(':')}"
        val config = LatchConfig()
        val keys = deriveLatchKeys(name, config, StringCodec.UTF8)
        connection.sync().del(*keys.all.toTypedArray())
        ...
        try {
            ...
            future.countDownAsync(futureGeneration, LatchRequestId.from("future-down-1"))
                .get(5.seconds).shouldBeInstanceOf<LatchMutationResult.Decremented>().remaining shouldBeEqualTo 1

            future.countDownAsync(futureGeneration, LatchRequestId.from("future-down-2"))
                .get(5.seconds) shouldBeEqualTo LatchMutationResult.Completed
            awaited.get(2.seconds) shouldBeEqualTo LatchAwaitResult.Completed

            future.inspectAsync(futureGeneration)
                .get(5.seconds).shouldBeInstanceOf<LatchCountResult.Completed>()

            future.deleteAsync(futureGeneration, LatchRequestId.from("future-delete"))
                .get(5.seconds) shouldBeEqualTo LatchMutationResult.Deleted

            val suspendGeneration = suspending.trySetCount(1, LatchRequestId.from("suspend-create"))
                .shouldBeInstanceOf<LatchSetCountResult.Created>().generation
            log.debug { "suspend-generation: $suspendGeneration" }

            suspending.getCount(suspendGeneration).shouldBeInstanceOf<LatchCountResult.Active>().count shouldBeEqualTo 1
            suspending.countDown(
                suspendGeneration,
                LatchRequestId.from("suspend-down")
            ) shouldBeEqualTo LatchMutationResult.Completed
            suspending.await(
                suspendGeneration,
                LatchRequestId.from("suspend-await"),
                Duration.ofSeconds(1),
            ) shouldBeEqualTo LatchAwaitResult.Completed
            suspending.inspect(suspendGeneration).shouldBeInstanceOf<LatchCountResult.Completed>()
            suspending.delete(
                suspendGeneration,
                LatchRequestId.from("suspend-delete")
            ) shouldBeEqualTo LatchMutationResult.Deleted
        } finally {
            future.close()
            suspending.close()
            connection.sync().del(*keys.all.toTypedArray())
