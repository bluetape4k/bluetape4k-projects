...
class ProducerSupportTest: AbstractPulsarTest() {
    ...
    @Test
    fun `producer DSL - Schema와 setup으로 Producer 생성`() = runSuspendIO {
    ...
    @Test
    fun `withProducer - 블록 실행 후 자동 close`() = runSuspendIO {
        ...
        try {
            client.withProducer(Schema.STRING, { topic(topic) }) {
                shouldNotBeNull()
                val msgId = sendSuspend("withProducer test").shouldNotBeNull()
    ...
    @Test
    fun `withProducer - compressionType 설정 동작`() = runSuspendIO {
        val client = newClient()
        val topic = newTopic()
        ...
        try {
            client.withProducer(Schema.STRING, {
                ...
                sendSuspend("compressed").shouldNotBeNull()
            }
