...
class OpenFgaCoroutinesTest {
    ...
    private val api = mockk<OpenFgaApi>()
    ...
    fun `공개 요청 값 객체는 직렬화 roundtrip을 지원한다`() {
        ...
        values.forEach { value ->
            ...
            val restored = BinarySerializers.Jdk.deserialize<Serializable>(bytes)
            restored shouldBeEqualTo value
        }
    }

    @Test
    fun `공개 요청 값 객체는 Fory 직렬화 roundtrip을 지원한다`() {
        val values = listOf(
            OpenFgaScope("store", "model"),
            openFgaTuple("user:anne", "reader", "document:budget"),
        )
        values.forEach { value ->
            ...
            val restored = BinarySerializers.FastFory.deserialize<Serializable>(bytes)
    ...
    fun `scope와 tuple builder는 공백 입력을 거부한다`() {
        ...
        assertFailsWith<IllegalArgumentException> {
            openFgaTuple("user:anne", " ", "document:budget")
        }
        assertFailsWith<IllegalArgumentException> {
            openFgaTuple(" ", "reader", "document:budget")
        }
    }

    @Test
    fun `checkSuspending은 명시한 store와 authorization model을 request에 고정하고 allow를 보존한다`() = runTest {
        val scope = OpenFgaScope("store-a", "model-a")
        val request = CheckRequest().tupleKey(
        ...
        every {
            api.check(
                "store-a",
                capture(bodySlot),
                any<ConfigurationOverride>()
            )
        } returns completableFutureOf(response)

        val actual = api.checkSuspending(scope, request)
        log.debug { "api response=$actual" }

        actual.data.allowed.shouldBeTrue()
        bodySlot.captured.authorizationModelId shouldBeEqualTo "model-a"
        bodySlot.captured.tupleKey.user shouldBeEqualTo "user:anne"
    ...
    fun `checkSuspending은 서로 다른 scope를 격리하고 request 옵션을 모두 보존한다`() = runTest {
        ...
        every {
            api.check(any(), any(), any<ConfigurationOverride>())
        } answers {
            captured += firstArg<String>() to secondArg<CheckRequest>()
            completableFutureOf(response)
        }

        val firstContextual = ContextualTupleKeys()
            .addTupleKeysItem(
        ...
        captured[0].second.authorizationModelId shouldBeEqualTo "model-first"
        captured[0].second.trace.shouldBeTrue()
        captured[0].second.contextualTuples shouldBeEqualTo firstContextual
        captured[0].second.context shouldBeEqualTo firstContext
        captured[0].second.consistency shouldBeEqualTo ConsistencyPreference.HIGHER_CONSISTENCY

        captured[1].second.authorizationModelId shouldBeEqualTo "model-second"
        captured[1].second.trace.shouldBeFalse()
        captured[1].second.context shouldBeEqualTo mapOf("tenant" to "second")
        captured[1].second.consistency shouldBeEqualTo ConsistencyPreference.MINIMIZE_LATENCY

        firstRequest.authorizationModelId shouldBeEqualTo "caller-model"
        secondRequest.authorizationModelId.shouldBeNull()
    }
    ...
    fun `checkSuspending은 호출자가 전달한 configuration override를 그대로 사용한다`() = runTest {
        ...
        val response = ApiResponse(
            ...
            emptyMap(),
            "",
            CheckResponse().allowed(false)
        )

        every {
            api.check("store", any(), refEq(override))
        } returns completableFutureOf(response)

        val scope = OpenFgaScope("store", "model")
        api.checkSuspending(scope, CheckRequest(), override)
    ...
    fun `checkSuspending은 원격 오류를 권한 허용으로 바꾸지 않는다`() = runTest {
        ...
        val actual = assertFailsWith<IllegalStateException> {
            val scope = OpenFgaScope("store", "model")
            api.checkSuspending(scope, CheckRequest())
        }
    ...
    fun `batchCheckSuspending은 항목별 허용과 오류를 모두 보존한다`() = runTest {
        ...
        every {
            api.batchCheck(any(), any(), any<ConfigurationOverride>())
        } returns completableFutureOf(response)
        ...
        val result = actual.data.result.shouldNotBeNull()
        result.forEach { (key, value) ->
            log.debug { "key=$key, result=$value" }
        }
        result["allowed"]?.allowed.shouldBeTrue()
        result["rejected"]?.error?.message shouldBeEqualTo "tuple rejected"
    }
    ...
    fun `readSuspending은 read filter와 consistency를 보존하고 page 옵션만 복사해 덮어쓴다`() = runTest {
        ...
        every {
            api.read("store-read", capture(captured), any<ConfigurationOverride>())
        } returns
                completableFutureOf(readResponse(emptyList(), ""))
        ...
        val copiedTuple = captured.captured.tupleKey.shouldNotBeNull()

        log.debug { "copiedTuple=$copiedTuple" }
        copiedTuple.user shouldBeEqualTo "user:anne"
        copiedTuple.relation shouldBeEqualTo "reader"
        copiedTuple.getObject() shouldBeEqualTo "document:budget"

        log.debug { "captured.captured=${captured.captured}" }
        captured.captured.pageSize shouldBeEqualTo 7
        captured.captured.continuationToken shouldBeEqualTo "cursor-2"
        captured.captured.consistency shouldBeEqualTo ConsistencyPreference.HIGHER_CONSISTENCY

        log.debug { "request=$request" }
        request.pageSize.shouldBeNull()
        request.continuationToken.shouldBeNull()
    }
    ...
    fun `readTuplesFlow는 같은 continuation token 반복을 거부한다`() = runTest {
        every {
            api.read(any(), any(), any<ConfigurationOverride>())
                ...
                completableFutureOf(readResponse(emptyList(), "same"))
    ...
    fun `readTuplesFlow는 장주기 cursor를 maxPages에서 중단한다`() = runTest {
        every {
            ...
            val token = secondArg<ReadRequest>().continuationToken
            val next = when (token) {
                null -> "a"
                "a" -> "b"
                else -> "a"
            }
            completableFutureOf(readResponse(emptyList(), next))
        }
    ...
    fun `writeSuspending은 model scope와 write delete 옵션을 보존하고 원본 request를 변경하지 않는다`() = runTest {
        ...
        every {
            api.write("store-a", capture(captured), any<ConfigurationOverride>())
        } returns completableFutureOf(response)

        api.writeSuspending(OpenFgaScope("store-a", "model-a"), request)

        log.debug { "captured.captured=${captured.captured}" }
        captured.captured.authorizationModelId shouldBeEqualTo "model-a"
        captured.captured.writes?.onDuplicate shouldBeEqualTo WriteRequestWrites.OnDuplicateEnum.IGNORE
        captured.captured.deletes?.onMissing shouldBeEqualTo WriteRequestDeletes.OnMissingEnum.IGNORE

        log.debug { "request=$request" }
        request.authorizationModelId shouldBeEqualTo "caller-model"
        request.writes?.onDuplicate shouldBeEqualTo WriteRequestWrites.OnDuplicateEnum.IGNORE
        request.deletes?.onMissing shouldBeEqualTo WriteRequestDeletes.OnMissingEnum.IGNORE
    }
