 ...
 *
 * 임베디드 Ignite 2.x 서버를 로컬에서 시작하고, 씬 클라이언트로 연결하여 테스트합니다.
 */
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
class IgniteClientNearCacheTest {

    companion object: KLogging() {
        private const val CACHE_NAME = "TEST_NEAR_CACHE"

        /** 테스트용 임베디드 Ignite 2.x 서버 노드 */
        val server: Ignite by lazy {
            igniteEmbedded {
                igniteInstanceName = "bt4k-near-cache-test-server"
            }.also {
                ShutdownQueue.register { it.close() }
            }
        }

        /** 임베디드 서버에 연결하는 Ignite 2.x 씬 클라이언트 */
        val igniteClient: IgniteClient by lazy {
            server  // 서버가 먼저 초기화되어야 함
