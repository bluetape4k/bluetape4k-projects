...
abstract class AbstractNearSuspendCacheTest
    ...
    fun `cache entry를 update하면, 다른 nearCache에서도 update 된다`() = runSuspendIO {
        ...
        await untilSuspending { nearSuspendCache2.get(key) == value2 }
    ...
    fun `두 개의 nearCoCache가 서로 변화가 반영된다`() = runSuspendIO {
        ...
        val actual2 = nearSuspendCache2.getAll(key1, key2).toList().associate { it.key to it.value }
        actual2 shouldContainSame expected

        val actual1 = nearSuspendCache1.getAll(key1, key2).toList().associate { it.key to it.value }
    ...
    fun `putAll with map - 복수의 cache entry를 추가하면 다른 nearCache에도 반영된다`() = runSuspendIO {
        ...
        await untilSuspending { keys.all { nearSuspendCache2.containsKey(it) } }

        nearSuspendCache2.getAll().toList() shouldContainSame map.entries.map {
            SuspendCacheEntry(
                it.key,
                it.value
            )
    ...
    fun `putIfAbsent - cache entry가 없는 경우에만 추가되고, 전파됩니다`() = runSuspendIO {
        ...
        await untilSuspending { nearSuspendCache1.containsKey(key2) }
    ...
    fun `replace - 모든 nearCache가 update 된다`() = runSuspendIO {
        ...
        await untilSuspending { nearSuspendCache1.get(key) == value2 }
    ...
    fun `get and replace - 기존 값을 가져오고 새로운 값으로 갱신한다`() = runSuspendIO {
        ...
        nearSuspendCache1.put(key, value)
        await untilSuspending { nearSuspendCache2.get(key) == value }
        ...
        await untilSuspending { nearSuspendCache2.get(key) == value3 }
    ...
    fun `removeAll - 모든 캐시를 삭제하면 nearCache들에게 반영된다`() = runSuspendIO {
        ...
        await untilSuspending { nearSuspendCache1.entries().count() == 0 }
