...
class SuspendSupplierExtensionsTest {
    ...
    inner class AndThenTests {
        ...
        @Test
        fun `andThen - 성공 시 결과를 변환한다`() = runTest {
        ...
        @Test
        fun `andThen with handler - 성공 시 result에 값이 전달된다`() = runTest {
            val fn: suspend () -> Int = { 42 }
        ...
        @Test
        fun `andThen with handler - 예외 발생 시 ex에 예외가 전달된다`() = runTest {
        ...
        @Test
        fun `andThen with resultHandler and exceptionHandler - 성공 시 resultHandler 호출`() = runTest {
        ...
        @Test
        fun `andThen with resultHandler and exceptionHandler - 예외 시 exceptionHandler 호출`() = runTest {
        ...
        @Test
        fun `andThen with handler - 취소 예외는 그대로 전파한다`() = runTest {
    ...
    inner class RecoverTests {
        ...
        @Test
        fun `recover with exceptionHandler - 예외가 발생하면 대체 값을 반환한다`() = runTest {
        ...
        @Test
        fun `recover with exceptionHandler - 예외가 없으면 원래 값을 반환한다`() = runTest {
            val fn: suspend () -> Int = { 42 }
            val safe = fn.recover { -1 }
        ...
        @Test
        fun `recover with resultPredicate - 조건 충족 시 대체 값을 반환한다`() = runTest {
        ...
        @Test
        fun `recover with resultPredicate - 조건 미충족 시 원래 값을 반환한다`() = runTest {
        ...
        @Test
        fun `recover with exceptionType - 해당 예외 타입이면 대체 값을 반환한다`() = runTest {
        ...
        @Test
        fun `recover with exceptionType - 다른 예외 타입이면 rethrow 된다`() = runTest {
        ...
        @Test
        fun `recover with exceptionTypes - 목록 예외 타입이면 대체 값을 반환한다`() = runTest {
            val fn: suspend () -> Int = { throw IOException("io error") }
            // 타입 파라미터 명시로 추론 해결
        ...
        @Test
        fun `recover with exceptionTypes - 목록 외 예외 타입이면 rethrow 된다`() = runTest {
        ...
        @Test
        fun `recover with exceptionHandler - 취소 예외는 복구하지 않고 전파한다`() = runTest {
            val fn: suspend () -> Int = { throw CancellationException("cancelled") }
            val safe = fn.recover { -1 }
        ...
        @Test
        fun `recover with exceptionType - 취소 예외는 타입이 일치해도 전파한다`() = runTest {
            val fn: suspend () -> Int = { throw CancellationException("cancelled") }
    ...
    inner class CompositionTests {
        ...
        @Test
        fun `andThen 후 recover 체인 동작 확인`() = runTest {
            val fn: suspend () -> Int = { 10 }
            val composed = fn
                .andThen { it * 2 }  // 20
        ...
        @Test
        fun `예외 발생 시 recover가 동작한다`() = runTest {
            var touched = false
            val fn: suspend () -> Int = { throw RuntimeException("err") }
        ...
        @Test
        fun `결과 predicate 로 조건부 분기가 동작한다`() = runTest {
        ...
        @Test
        fun `정상 결과는 predicate 분기에 걸리지 않는다`() = runTest {
            val fn: suspend () -> Int = { 42 }
            val composed = fn.recover(
                resultPredicate = { it == 0 },
                resultHandler = { 99 }
