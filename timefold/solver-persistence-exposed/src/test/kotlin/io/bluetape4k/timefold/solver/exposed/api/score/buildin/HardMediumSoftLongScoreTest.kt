...
class HardMediumSoftLongScoreTest: AbstractScoreExposedTest() {
    ...
    class E1(id: EntityID<Int>): IntEntity(id) {
        ...
        var name by T1.name
        var hardMediumSoftLongScore by T1.hardMediumSoftLongScore

        override fun equals(other: Any?): Boolean = idEquals(other)
        override fun hashCode(): Int = idHashCode()
        override fun toString(): String = entityToStringBuilder()
            .add("name", name)
            .add("hardMediumSoftLongScore", hardMediumSoftLongScore)
            .toString()
    }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `HardMediumSoftLongScore 수형의 속성을 가진 엔티티 사용`(testDB: TestDB) {
        withTables(testDB, T1) {
            // Creates entity with hard medium soft long score
            val saved = E1.new {
                this.name = faker.name().name()
                this.hardMediumSoftLongScore = HardMediumSoftLongScore.of(
                    faker.random().nextLong(),
                    faker.random().nextLong(),
                    faker.random().nextLong()
                )
