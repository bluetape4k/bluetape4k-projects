 ...
 * - 타임아웃을 두지 않으므로 collector가 끝내 등록되지 않으면 계속 대기합니다.
 ...
 * - 타임아웃을 두지 않으므로 목표 수에 도달하지 않으면 계속 대기합니다.
 *
 * ```kotlin
 * val subject = PublishSubject<Int>()
 * launch { subject.collect {} }
...
/**
 * 두 값을 `Any`로 강제 캐스팅한 뒤 동등 비교합니다.
 *
 * ## 동작/계약
 * - 내부적으로 `(left as Any) == (right as Any)`를 수행합니다.
 * - `left` 또는 `right`가 `null`이면 `NullPointerException`이 발생합니다.
 ...
 * ```
 *
 * @param left 비교할 왼쪽 값입니다. `null`이면 예외가 발생합니다.
 * @param right 비교할 오른쪽 값입니다. `null`이면 예외가 발생합니다.
 */
