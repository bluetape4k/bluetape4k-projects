 ...
 * }
 */
fun <T: Any> Iterable<T?>.whenAnyNotNull(block: (List<T?>) -> Unit) {
    if (any { it != null }) {
        block(this@whenAnyNotNull.toList())
    }
}
 ...
 * ```
 */
inline fun <T: Any> coalesce(vararg options: T?): T? = options.find { it != null }
 ...
 * ```
 */
inline fun <T: Any> Iterable<T?>.coalesce(): T? = find { it != null }
