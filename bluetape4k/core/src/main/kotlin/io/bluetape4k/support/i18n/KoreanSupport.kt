...
fun String.containKorean(): Boolean {
    ...
    if (!exists) {
        exists = firstOrNull { it.code in KOREAN_RANGE } != null
    }
    return exists
}
...
/**
 * 문자열 내에서 한글 자소를 분해하여 초/중/종성 으로 제공합니다.
 *
 * ```
 * 예: 한국 -> ㅎㅏㄴㄱㅜㄱ
 * ```
...
fun String.getJasoLetter(): String {
    ...
    repeat(length) { i ->
        ...
        if (char in KOREAN_RANGE) {
            val (choIndex, jungIndex, jongIndex) = char.getIndexes()
...
fun String.getChosung(): CharArray {
    ...
    repeat(length) { i ->
        val char = this[i].code
        if (char in KOREAN_RANGE) {
            val (choIndex, _, _) = char.getIndexes()
