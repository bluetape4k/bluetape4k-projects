...
const edges = [
  ...
  { points: [[1740, 595], [1740, 489], [1726, 475], [749, 475], [735, 461], [735, 400]], tone: "event", dashed: true, label: "invalidate peer front", labelAt: [1238, 447] },
  { points: [[275, 755], [275, 836], [289, 850], [491, 850], [505, 864], [505, 910]], tone: "event", label: "optional", labelAt: [390, 822] },
  { points: [[1240, 755], [1240, 836], [1226, 850], [1119, 850], [1105, 864], [1105, 910]], tone: "option", dashed: true, label: "factory", labelAt: [1170, 822] },
...
function card(id) {
  ...
  return `<g id="${id}"><rect class="card" x="${c.x}" y="${c.y}" width="${c.w}" height="${c.h}" rx="8" fill="${p.fill}" stroke="${p.stroke}"/><text class="kicker" x="${c.x + 34}" y="${c.y + 28}">${esc(c.kicker)}</text><text class="cardTitle" x="${c.x + 28}" y="${c.y + 60}">${esc(c.title)}</text><path class="divider" d="M${c.x} ${c.y + 78}H${c.x + c.w}" stroke="${p.dark}"/>${c.lines.map((line, i) => `<text class="body" x="${c.x + 34}" y="${c.y + 106 + i * 21}">${esc(line)}</text>`).join("")}</g>`;
}
...
const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" role="img" aria-label="Hazelcast near-cache read write and invalidation flow" data-intent="Hazelcast NearCache keeps the Caffeine front cache responsive while routing misses and writes through IMap, then invalidating peer-local entries from remote member events." data-evidence="cache/cache-hazelcast/README.md,cache/cache-hazelcast/src/main/kotlin/io/bluetape4k/cache/nearcache/HazelcastNearCache.kt" data-source-read="cache/cache-hazelcast/README.md; cache/cache-hazelcast/src/main/kotlin/io/bluetape4k/cache/nearcache/HazelcastEntryEventListener.kt" data-allow-grid="true">
...
<rect class="lane" x="65" y="165" width="1915" height="285" rx="8"/><text class="laneTitle" x="92" y="198">read-through lane</text><text class="laneText" x="92" y="222">front hit returns immediately; front miss reads IMap and refills the local Caffeine tier</text>
<rect class="lane" x="65" y="520" width="1915" height="285" rx="8"/><text class="laneTitle" x="92" y="553">write-through and coherence lane</text><text class="laneText" x="92" y="577">writes update local and remote tiers; remote member changes travel back through the EntryListener</text>
<rect class="lane" x="65" y="850" width="1915" height="250" rx="8"/><text class="laneTitle" x="92" y="883">side variants and lifecycle</text>
