...
function card({x,y,w,h,c,title,sub,lines}){const [f,s]=C[c];return `<g><rect class="card" x="${x}" y="${y}" width="${w}" height="${h}" rx="8" fill="${f}" stroke="${s}"/><text class="cardTitle" x="${x+28}" y="${y+48}">${esc(title)}</text><text class="sub" x="${x+30}" y="${y+78}">${esc(sub)}</text><path d="M${x} ${y+98}H${x+w}" stroke="${s}" opacity=".5"/>${lines.map((l,i)=>`<text class="line" x="${x+30}" y="${y+134+i*30}">${esc(l)}</text>`).join("")}</g>`}
function edge(points,c,label,at){const [,,d]=C[c];const path=points.map((p,i)=>`${i?"L":"M"}${p[0]} ${p[1]}`).join(" ");const q=at??points[Math.floor(points.length/2)];return `<g><path class="edge dashed" d="${path}" stroke="${d}" marker-end="url(#open-${c})"/><text class="edgeLabel" x="${q[0]+8}" y="${q[1]-8}">${esc(label)}</text></g>`}
const body=[
  card({x:95,y:215,w:620,h:245,c:"green",title:"LettuceCaches factory",sub:"module entrypoint",lines:["nearJCache(): LettuceJCache back cache", "suspendNearJCache(): Caffeine front + LettuceSuspendJCache back", "codec defaults to LettuceBinaryCodecs.default()"]}),
  card({x:850,y:200,w:620,h:270,c:"blue",title:"NearJCache<K,V>",sub:"cache-core blocking two-tier type",lines:["implements JCache<K,V> by backCache", "get() reads local front only", "getDeeply() reads back and fills front", "put/remove/replace update front then back"]}),
  card({x:1600,y:200,w:600,h:270,c:"teal",title:"SuspendNearJCache<K,V>",sub:"cache-core coroutine two-tier type",lines:["implements SuspendJCache<K,V> by backCache", "get() delegates to getDeeply()", "Flow reads expose local front entries", "withoutListener() handles listener-free backends"]}),
  card({x:185,y:710,w:580,h:255,c:"orange",title:"NearJCacheConfig DSL",sub:"configuration boundary",lines:["cacheName identifies Redis hash/front cache", "frontCacheConfiguration defaults to Caffeine", "isSynchronous + syncRemoteTimeout tune event delivery"]}),
  card({x:915,y:710,w:560,h:255,c:"slate",title:"LettuceJCache back",sub:"Redis hash storage",lines:["hset/hget/hdel through LettuceMap", "dispatches CREATED/UPDATED/REMOVED events", "close() releases resources without deleting data"]}),
  card({x:1620,y:710,w:520,h:255,c:"pink",title:"Local front cache",sub:"Caffeine-backed local tier",lines:["fast local reads", "back-cache listener propagates peer changes", "clearLocal() keeps Redis data intact"]}),
  edge([[715,335],[850,335]],"blue","creates blocking",[735,318]),
  edge([[405,215],[405,160],[1900,160],[1900,200]],"teal","creates coroutine",[1130,147]),
  edge([[1160,470],[1160,710]],"slate","delegates to back"),
  edge([[1900,470],[1900,710]],"pink","uses front"),
  edge([[475,460],[475,710]],"orange","DSL config"),
  edge([[1475,835],[1620,835]],"pink","events fill/evict"),
];
