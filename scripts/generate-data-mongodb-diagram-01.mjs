...
const layers = [
  { x: 70, y: 165, w: 1660, h: 350, title: "Native coroutine driver types", note: "Runtime still belongs to MongoDB Kotlin Coroutine Driver: suspend CRUD and Flow-returning query APIs are not wrapped." },
  { x: 70, y: 555, w: 1660, h: 420, title: "bluetape4k additions", note: "Factories, cached clients, focused extensions, BSON helpers, and ordered pipeline stage builders." },
  { x: 70, y: 1010, w: 1660, h: 96, title: "Design contract", note: "Add only missing convenience. Keep native aggregate(), collection types, driver exceptions, and coroutine Flow behavior visible." },
];

const cards = {
  settings: { x: 120, y: 300, w: 330, h: 146, fill: "#EFF6FF", stroke: c.blue, title: "MongoClientSettings", tag: "<<driver>>", body: ["builder configures sockets", "connection string, codecs"] },
  client: { x: 535, y: 300, w: 330, h: 146, fill: "#F0FDF4", stroke: c.green, title: "MongoClient", tag: "<<coroutine driver>>", body: [{ code: "listDatabaseNames()" }, { code: "startSession()" }] },
  database: { x: 950, y: 300, w: 330, h: 146, fill: "#F0FDFA", stroke: c.teal, title: "MongoDatabase", tag: "<<coroutine driver>>", body: [{ code: "getCollection(name, Class)" }, { code: "listCollectionNames()" }] },
  collection: { x: 1365, y: 300, w: 330, h: 146, fill: "#FAF5FF", stroke: c.purple, title: "MongoCollection<T>", tag: "<<coroutine driver>>", body: ["native suspend CRUD", { code: "find(), aggregate()" }] },
  provider: { x: 120, y: 750, w: 340, h: 172, fill: "#EFF6FF", stroke: c.blue, title: "MongoClientProvider", tag: "<<object>>", body: [{ code: "getOrCreate(url)" }, { code: "getOrCreate(settings)" }, "ConcurrentHashMap caches"] },
  factory: { x: 510, y: 750, w: 340, h: 172, fill: "#F0FDF4", stroke: c.green, title: "Client factory APIs", tag: "<<top-level functions>>", body: [{ code: "mongoClient { ... }" }, { code: "mongoClientOf(url)" }, "delegates to MongoClient.create"] },
  extensions: { x: 900, y: 750, w: 360, h: 172, fill: "#F0FDFA", stroke: c.teal, title: "Mongo extension APIs", tag: "<<extension functions>>", body: [{ code: "getCollectionOf<T>(name)" }, { code: "findFirst / exists / upsert" }, { code: "findAsFlow(filter, sort)" }] },
  dsl: { x: 1310, y: 750, w: 370, h: 172, fill: "#FFF7ED", stroke: c.orange, title: "BSON + pipeline DSL", tag: "<<builders>>", body: [{ code: "documentOf { ... }" }, { code: "getAs<T>(key)" }, { code: "pipeline { add(stage) }" }] },
};
...
const flows = [
  ...
  { color: c.orange, d: "M1495 750 L1495 682 L1530 682 L1530 446", label: "feeds aggregate", x: 1518, y: 662, dash: true, open: true },
];
