...
function rankingChart({
  ...
  ${bar(plotX, y, plotW, max, row.value, row.color ?? series[index % series.length], "", {
            unit: ` ${unit}`,
            digits: row.digits ?? 0
        })}
</g>`;
...
function groupedBarsChart({name, title, subtitle, source, unit, max, ticks, groups, seriesLabels, note, width = 1760}) {
  ...
  ${group.values.map((value, index) => bar(plotX, groupY + index * 22, plotW, max, value, series[index % series.length], `${seriesLabels[index]} `, {
            unit: ` ${unit}`,
            digits: group.digits ?? 0,
            height: 15
        })).join("\n")}
</g>`;
    }).join("\n");
...
function splitPanelsChart({name, title, subtitle, source, panels, note, width = 1800}) {
  ...
  ${bar(plotX, y, plotW, panel.max, row.value, row.color ?? series[index % series.length], "", {
                unit: ` ${panel.unit}`,
                digits: panel.digits ?? 0
            })}
</g>`;
        }).join("\n")}
</g>`;
...
function compactPanel({x, y, w, h, title, unit, max, ticks, rows}) {
  ...
  ${bar(plotX, rowY, plotW, max, row.value, row.color ?? series[index % series.length], "", {
            unit: ` ${unit}`,
            height: 13
        })}
</g>`;
...
${compactPanel({
        ...
        w: panelW,
        h: panelH,
        title: "Single-thread batch=100",
        unit: "ops/s",
        max: 450000,
        ticks: [0, 150000, 300000, 450000],
        rows: singleSmall
    })}
${compactPanel({
        x: 950,
        ...
        w: panelW,
        h: panelH,
        title: "Concurrent batch=100",
        unit: "ops/s",
        max: 90000,
        ticks: [0, 30000, 60000, 90000],
        rows: concurrentSmall
    })}
${compactPanel({
        x: 70,
        y: 650,
        w: panelW,
        h: panelH,
        title: "Single-thread batch=10000",
        unit: "ops/s",
        max: 4500,
...
${compactPanel({
        ...
        w: panelW,
        h: panelH,
        title: "Concurrent batch=10000",
        unit: "ops/s",
        max: 850,
        ticks: [0, 250, 500, 750],
        rows: concurrentLarge
    })}
<g>
  <rect class="note" x="94" y="${bottom.noteY}" width="${width - 188}" height="${noteH}" rx="16"/>
