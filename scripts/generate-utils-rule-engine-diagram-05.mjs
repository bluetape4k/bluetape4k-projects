...
const svgPath="docs/images/readme-diagrams/utils-rule-engine-diagram-05.svg", pngPath="docs/images/readme-diagrams/utils-rule-engine-diagram-05.png";
const W=1780,H=1480;
...
const tones={input:["#EFF6FF",C.blue,"#1D4ED8"],loop:["#F0FDFA",C.teal,"#0F766E"],exec:["#FAF5FF",C.purple,"#6D28D9"],facts:["#F0FDF4",C.green,"#15803D"],stop:["#FFF7ED",C.orange,"#C2410C"],out:["#F8FAFC",C.gray,"#475569"]};
const cards={
  start:{x:610,y:190,w:560,h:205,tone:"input",title:"InferenceRuleEngine.fire()",kicker:"entrypoint",lines:["accepts RuleSet + Facts","keeps DefaultRuleEngine delegate","starts with all configured rules"]},
  select:{x:610,y:480,w:560,h:225,tone:"loop",title:"selectCandidates()",kicker:"evaluation pass",lines:["evaluate every rule against current Facts","keep only rules that return true","TreeSet preserves priority order"]},
  any:{x:640,y:790,w:500,h:195,tone:"stop",title:"any candidates?",kicker:"loop condition",lines:["empty set stops the chain","non-empty set executes one cycle"]},
  fire:{x:610,y:1070,w:560,h:235,tone:"exec",title:"delegate.doFire()",kicker:"execution pass",lines:["wrap selected rules as RuleSet","execute selected rules in priority order","actions mutate the same Facts"]},
  facts:{x:80,y:1045,w:390,h:245,tone:"facts",title:"Facts changes",kicker:"working memory",lines:["new or changed facts may satisfy more rules","same Facts instance flows into next cycle","no snapshot is created"]},
  done:{x:1260,y:790,w:390,h:220,tone:"out",title:"no candidates remain",kicker:"terminal state",lines:["last Facts state is the result","engine returns to caller","check() still delegates once"]},
};
const edges=[
...
function marker(){return Object.entries(tones).map(([k,t])=>`<marker id="arrow-${k}" markerWidth="24" markerHeight="18" refX="22" refY="9" orient="auto" markerUnits="userSpaceOnUse"><path d="M2 2 L22 9 L2 16 Z" fill="${t[2]}"/></marker>`).join("")}
...
function path(p){return p.map((q,i)=>`${i?"L":"M"}${q[0]} ${q[1]}`).join(" ")}
function edge(e){const t=tones[e[2]],w=Math.max(94,e[4].length*8+26);return `<path class="edge" d="${path(e[3])}" stroke="${t[2]}" marker-end="url(#arrow-${e[2]})"/><rect class="edgeLabelBg" x="${e[5][0]-8}" y="${e[5][1]-18}" width="${w}" height="26" rx="6"/><text class="edgeLabel" x="${e[5][0]}" y="${e[5][1]}">${esc(e[4])}</text>`}
