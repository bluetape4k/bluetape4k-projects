...
const edgeSpecs = [
    ...
    },
    {
        from: "UniPipeline",
        to: "UniTerminal",
        points: [[1780, 355], [1940, 355]],
        color: "teal",
        label: "terminal choice",
        labelAt: [1860, 334]
    },
    {
        from: "MultiSources",
        to: "MultiHelpers",
        points: [[540, 780], [700, 780]],
        color: "violet",
        label: "converted by",
        labelAt: [620, 759]
    },
    {
        from: "MultiHelpers",
        to: "MultiPipeline",
        points: [[1160, 780], [1320, 780]],
        color: "amber",
        label: "produces Multi",
        labelAt: [1240, 759]
    },
    {
        from: "MultiPipeline",
        to: "MultiTerminal",
        points: [[1780, 780], [1940, 780]],
        color: "teal",
        label: "terminal choice",
        labelAt: [1860, 759]
    },
    {
        from: "CoroutineBlock",
        to: "CoroutineBridge",
        points: [[940, 1205], [1110, 1205]],
        color: "teal",
        label: "async",
        labelAt: [1025, 1184]
    },
    {
        from: "CoroutineBridge",
        to: "InteropResult",
        points: [[1630, 1205], [1800, 1205]],
        color: "teal",
        label: "asUni",
        labelAt: [1715, 1184]
    },
    {
        from: "InteropResult",
        to: "UniPipeline",
        points: [[2060, 1095], [2060, 990], [2520, 990], [2520, 530], [1550, 530], [1550, 470]],
        color: "blue",
        dashed: true,
        label: "uses Uni pipeline",
        labelAt: [2175, 971]
    },
];
...
const body = [
    ...
    card({
        ...
        color: "blue",
        kicker: "source",
        title: "single result inputs",
        lines: ["item, supplier, failure", "CompletionStage", "Future with timeout"],
        footer: "0 or 1 item intent"
    }),
    card({
        id: "UniHelpers",
        x: 700,
        ...
        title: "Uni creation helpers",
        lines: ["uniOf(...)", "uniFailureOf(...)", "uniCompletionStageOf(...)", "uniFutureOf(...)"],
        footer: "creates Uni<T>"
    }),
    card({
        id: "UniPipeline",
        x: 1320,
        ...
        h: 230,
        color: "teal",
        kicker: "Mutiny pipeline",
        title: "Uni<T> processing",
        lines: ["onEach { callback }", "onItem().invoke(...)", "failure propagates downstream"],
        footer: "item is passed through unchanged"
    }),
    card({
        id: "UniTerminal",
        x: 1940,
        ...
        h: 230,
        color: "slate",
        kicker: "terminal boundary",
        title: "await or CompletionStage",
        lines: ["await().atMost(...)", "await().indefinitely()", "subscribeAsCompletionStage()"],
        footer: "caller observes T, null, or failure"
    }),
    ...
    card({
        ...
        kicker: "source",
        title: "stream inputs",
        lines: ["vararg and range", "Iterable, Sequence, Stream", "arrays and progressions", "deferred Uni or CompletionStage repeats"],
        footer: "0 or more items intent"
    }),
    card({
        id: "MultiHelpers",
        x: 700,
        ...
        title: "Multi creation helpers",
        lines: ["multiOf(...)", "multiRangeOf(...)", "asMulti() conversions", "deferUni / deferCompletionStage"],
        footer: "creates Multi<T> or UniRepeat<T>"
    }),
    card({
        id: "MultiPipeline",
        x: 1320,
        ...
        color: "teal",
        kicker: "Mutiny pipeline",
        title: "Multi<T> processing",
        lines: ["onEach { callback }", "onItem().invoke(...)", "source order is preserved", "callback failure fails stream"],
        footer: "items continue downstream"
    }),
    card({
        id: "MultiTerminal",
        x: 1940,
        ...
        color: "slate",
        kicker: "terminal boundary",
        title: "collect or subscribe",
        lines: ["collect().asList()", "await().indefinitely()", "subscriber consumes stream"],
        footer: "caller observes list, stream, or failure"
    }),
    ...
    card({
        ...
        h: 220,
        color: "teal",
        kicker: "CoroutineScope",
        title: "suspend block",
        lines: ["CoroutineScope.asUni { ... }", "runs async in current scope", "does not change receiver scope"],
        footer: "source is coroutine code"
    }),
    card({
        id: "CoroutineBridge",
        x: 1110,
        ...
        h: 220,
        color: "teal",
        kicker: "kotlinx.coroutines.mutiny",
        title: "async(...).asUni()",
        lines: ["Deferred result becomes Uni", "block exception becomes failed Uni", "cancellation follows coroutine machinery"],
        footer: "bridge targets Uni<T>"
    }),
    card({
        id: "InteropResult",
        x: 1800,
        ...
        h: 220,
        color: "blue",
        kicker: "Mutiny result",
        title: "Uni<T>",
        lines: ["continues through normal Uni pipeline", "can use onEach", "can await or subscribe"],
        footer: "same terminal surface as UniSupport"
    }),
