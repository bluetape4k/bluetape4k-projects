...
const top = [
  { key: "ds", x: 110, y: 235, w: 295, h: 92, fill: "#EFF6FF", stroke: c.blue, title: "DataSource", sub: "connection provider" },
  { key: "conn", x: 435, y: 235, w: 295, h: 92, fill: "#F0FDF4", stroke: c.green, title: "Connection", sub: "statement + transaction scope" },
  { key: "stmt", x: 760, y: 235, w: 295, h: 92, fill: "#FFF7ED", stroke: c.orange, title: "PreparedStatement", sub: "parameter binding + batch" },
  { key: "rs", x: 1085, y: 235, w: 295, h: 92, fill: "#FAF5FF", stroke: c.purple, title: "ResultSet", sub: "typed reads + row mapping" },
];
...
function topNode(n) {
  return `<g><rect class="node" x="${n.x}" y="${n.y}" width="${n.w}" height="${n.h}" rx="8" fill="${n.fill}" stroke="${n.stroke}"/><text class="nodeTitle" x="${n.x + n.w / 2}" y="${n.y + 38}" text-anchor="middle">${esc(n.title)}</text><text class="nodeSub" x="${n.x + n.w / 2}" y="${n.y + 68}" text-anchor="middle">${esc(n.sub)}</text></g>`;
}
...
function groupCard(g) {
  return `<g><rect class="node" x="${g.x}" y="${g.y}" width="${g.w}" height="${g.h}" rx="8" fill="${g.fill}" stroke="${g.stroke}"/><text class="cardTitle" x="${g.x + g.w / 2}" y="${g.y + 38}" text-anchor="middle">${esc(g.title)}</text><line x1="${g.x}" y1="${g.y + 58}" x2="${g.x + g.w}" y2="${g.y + 58}" stroke="${g.stroke}" stroke-opacity=".35"/>${g.lines.map((line, i) => {
    const y = g.y + 88 + i * 27;
...
<rect class="lane" x="82" y="160" width="${W - 164}" height="225" rx="10"/>
...
<rect class="lane" x="82" y="420" width="${W - 164}" height="385" rx="10"/>
...
<rect class="lane" x="82" y="840" width="${W - 164}" height="86" rx="10"/>
