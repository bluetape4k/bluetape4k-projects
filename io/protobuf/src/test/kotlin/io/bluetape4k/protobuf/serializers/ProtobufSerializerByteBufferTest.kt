...
class ProtobufSerializerByteBufferTest {
    ...
    fun `deserialization preserves bounded heap direct sliced and read only source state`() {
        val wire = serializer.serialize(message)
        val heap = ByteBuffer.allocate(wire.size + 4).apply {
            position(2)
            put(wire)
            flip()
            position(2)
        }
        val direct = ByteBuffer.allocateDirect(wire.size + 4).apply {
            position(2)
            put(wire)
            flip()
            position(2)
        }
        val sliced = ByteBuffer.wrap(ByteArray(2) + wire + ByteArray(2)).apply {
            position(2)
            limit(2 + wire.size)
        }
            .slice()
    ...
    fun `raw preflight failures and null preserve target state`() {
        ...
        assertFailsWith<BufferOverflowException> {
            serializer.serializeTo(message, full)
        }
    ...
    fun `read only target precedes protobuf null strict and trusted dispatch`() {
        ...
        listOf<Pair<ProtobufSerializer, Any?>>(
            ...
            assertFailsWith<ReadOnlyBufferException> {
                subject.serializeTo(value, target)
            }
    ...
    fun `trusted fallback overflow calls fallback once and preserves target`() {
        ...
        assertFailsWith<BufferOverflowException> {
            ProtobufSerializer(fallback).serializeTo(FallbackValue(1), target)
        }
    ...
    fun `public constructor and factory retain trusted fallback parity`() {
        ...
        listOf(
            ...
            ProtobufSerializer(fallback = BinarySerializers.FastFory),
            ProtobufSerializer.trustedInternalProtobuf(BinarySerializers.FastFory),
        ).forEach { trusted ->
            ...
            val actual = ByteArray(bytes.size).also {
                target.duplicate().apply { flip() }.get(it)
            }
            actual shouldContentEqual bytes
