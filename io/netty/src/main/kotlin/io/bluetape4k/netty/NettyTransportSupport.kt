 ...
 *
 * ```kotlin
 * val epollAvailable = isPresentNettyTransportNativeEpoll()
 * // epollAvailable == true  (Linux 환경에서 netty-transport-native-epoll 의존성이 있을 때)
 * ```
 *
 * @return Boolean
 */
 ...
 *
 * ```kotlin
 * val kqueueAvailable = isPresentNettyTransportNativeKQueue()
 * // kqueueAvailable == true  (macOS 환경에서 netty-transport-native-kqueue 의존성이 있을 때)
 * ```
 *
 * @return Boolean
 */
