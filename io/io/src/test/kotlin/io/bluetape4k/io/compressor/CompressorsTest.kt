...
class CompressorsTest {
    ...
    @Nested
    @DisplayName("Zip")
    inner class ZipCompressorRoundTripTest: AbstractCompressorTest() {
        override val compressor: Compressor = ZipCompressor()
    }
    ...
    @Test
    fun `BZip2Compressor 는 0 이하 bufferSize 를 허용하지 않는다`() {
        assertFailsWith<IllegalArgumentException> {
            BZip2Compressor(0)
        }
        assertFailsWith<IllegalArgumentException> {
