 ...
 */
suspend fun Buffer.readAll(sink: SuspendSink): Long {
    val size = this.size
 ...
 * @return 쓰여진 바이트 수, `source`가 소진된 경우 0
 */
suspend fun BufferedSource.readAll(sink: SuspendSink): Long {
 ...
 */
suspend fun BufferedSink.writeAll(source: SuspendSource): Long {
 ...
 * @return 쓰여진 바이트 수, `source`가 소진된 경우 0
 */
suspend fun BufferedSuspendSource.readAll(sink: Sink): Long {
    var totalBytesWritten = 0L
    while (read(this.buffer, SEGMENT_SIZE) != -1L) {
        val emitByteCount = buffer.completeSegmentByteCount()
        if (emitByteCount > 0L) {
 ...
 */
suspend fun BufferedSuspendSink.writeAll(source: Source): Long {
    var totalBytesRead = 0L
    while (true) {
        val readCount = source.read(this.buffer, SEGMENT_SIZE)
        if (readCount == -1L) break
 ...
 */
suspend fun BufferedSuspendSink.write(source: Source, byteCount: Long): BufferedSuspendSink {
