 ...
 *
 * ## 동작/계약
 * - 수신 객체가 null이면 빈 바이트 배열([emptyByteArray])을 반환합니다.
 * - null이 아니면 `JSONB.toBytes(this, context)`를 호출합니다.
 * - 직렬화 옵션은 [context] 설정을 따릅니다.
 * - 직렬화 실패 시 `com.alibaba.fastjson2.JSONException`이 전파됩니다.
 *
 * ```kotlin
...
fun Any?.toJsonBytes(context: JSONWriter.Context? = null): ByteArray =
    this?.let { JSONB.toBytes(it, context) } ?: emptyByteArray
 ...
 * - 타입 불일치/파싱 실패 시 `com.alibaba.fastjson2.JSONException`이 전파됩니다.
 *
 * ```kotlin
 ...
 * - **주의**: 스트림 전체를 메모리에 적재합니다. 대용량 데이터에서는 `JSONB.parseObject` 스트리밍 API를 직접 사용하세요.
 * - 파싱 실패 시 `com.alibaba.fastjson2.JSONException`이 전파됩니다.
 *
 * ```kotlin
