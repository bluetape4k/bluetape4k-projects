...
class VersionedCiphertextSupportTest: AbstractTinkTest() {
    ...
    fun `버전과 암호문을 pack 후 unpack하면 원래 값 복원`() {
        val version = 42L
        val ciphertext = faker.lorem().paragraph().toUtf8Bytes()
    ...
    fun `버전 1부터 여러 버전 pack unpack 확인`() {
        listOf(1L, 2L, 100L, Long.MAX_VALUE).forEach { version ->
            val data = "version-$version".toUtf8Bytes()
    ...
    fun `packed 크기는 항상 Long SIZE_BYTES 보다 크다`() {
        ...
        packed.size shouldBeGreaterThan Long.SIZE_BYTES
    }
