...
class SuspendRecordWriterSupportTest {
    ...
    inner class CSV {
        @Test
        fun `File에 CSV 레코드를 기록한다`() = runSuspendIO {
        ...
        @Test
        fun `File에 CSV 레코드를 헤더 없이 기록한다`() = runSuspendIO {
        ...
        @Test
        fun `File에 엔티티를 CSV 레코드로 변환하여 기록한다`() = runSuspendIO {
        ...
        @Test
        fun `CSV 파일에 기록하고 다시 읽는다`() = runSuspendIO {
        ...
        @Test
        fun `writeAll로 Iterable 데이터를 기록한다`() = runSuspendIO {
            StringWriter().use { sw ->
        ...
        @Test
        fun `writeAll로 Iterable 엔티티를 변환하여 기록한다`() = runSuspendIO {
            StringWriter().use { sw ->
                SuspendCsvRecordWriter(sw).use { writer ->
        ...
        @Test
        fun `writeRow로 entity를 변환하여 기록한다`() = runSuspendIO {
            StringWriter().use { sw ->
                SuspendCsvRecordWriter(sw).use { writer ->
                    writer.writeHeaders("name", "age", "city")
                    people.forEach { person ->
                        writer.writeRow(person) { p ->
                            listOf(p.name, p.age, p.city)
                        }
                    }
                }
    ...
    inner class TSV {
        ...
        @Test
        fun `File에 TSV 레코드를 기록한다`() = runSuspendIO {
        ...
        @Test
        fun `File에 엔티티를 TSV 레코드로 변환하여 기록한다`() = runSuspendIO {
        ...
        @Test
        fun `TSV 파일에 기록하고 다시 읽는다`() = runSuspendIO {
