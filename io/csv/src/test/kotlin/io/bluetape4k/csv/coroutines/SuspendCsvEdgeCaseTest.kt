...
class SuspendCsvEdgeCaseTest {
    ...
    @Test
    fun `빈 입력 스트림에서 suspend CSV 읽기는 빈 Flow 를 반환한다`() = runSuspendIO {
    ...
    @Test
    fun `빈 입력 스트림에서 suspend TSV 읽기는 빈 Flow 를 반환한다`() = runSuspendIO {
        val emptyInput = ByteArrayInputStream(ByteArray(0))
    ...
    @Test
    fun `헤더 행만 있는 CSV 에서 skipHeader=true 이면 빈 Flow 를 반환한다`() = runSuspendIO {
    ...
    @Test
    fun `헤더 행만 있는 TSV 에서 skipHeader=true 이면 빈 Flow 를 반환한다`() = runSuspendIO {
    ...
    @Test
    fun `쉼표가 포함된 필드를 suspend CSV 로 쓰고 읽으면 올바르게 파싱된다`() = runSuspendIO {
    ...
    @Test
    fun `큰따옴표가 포함된 필드를 suspend CSV 로 쓰고 읽으면 올바르게 파싱된다`() = runSuspendIO {
    ...
    @Test
    fun `유니코드 문자를 포함한 suspend CSV 를 쓰고 읽으면 올바르게 파싱된다`() = runSuspendIO {
    ...
    @Test
    fun `null 값이 포함된 CSV 를 suspend 로 쓰고 읽으면 null 로 파싱된다`() = runSuspendIO {
    ...
    @Test
    fun `suspend CSV 파일에 쓰고 다시 읽으면 정확한 값이 반환된다`() = runSuspendIO {
    ...
    @Test
    fun `suspend TSV 파일에 쓰고 다시 읽으면 정확한 값이 반환된다`() = runSuspendIO {
    ...
    @Test
    fun `엔티티를 suspend CSV 로 쓰고 읽으면 정확한 값이 반환된다`() = runSuspendIO {
        data class Item(
            val id: Int,
            val label: String,
        )
    ...
    @Test
    fun `단일 컬럼 suspend CSV 를 쓰고 읽으면 정확히 반환된다`() = runSuspendIO {
    ...
    @Test
    fun `transform 을 사용한 suspend CSV 읽기가 정확한 값을 반환한다`() = runSuspendIO {
