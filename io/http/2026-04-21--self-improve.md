...
**변경
이유**: `@Threads(100)` JMH 벤치마크에서 각 스레드가 50ms 요청을 보낼 때, 기본 Tomcat 스레드 200개가 서버 측 병목이 되어 클라이언트 성능 측정이 왜곡됨. Docker 이미지 재빌드 (
...
순수 처리량 (ops/s)만 보면 HC5 Classic과 OkHttp3가 비슷하지만, **CPU 점유율 및 리소스 효율** 관점에서는 차이가 있다:
