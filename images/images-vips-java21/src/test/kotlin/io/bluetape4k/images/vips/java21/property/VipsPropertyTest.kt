package io.bluetape4k.images.vips.java21.property
...
/**
 * vips 이미지 연산의 불변식(property-based) 테스트.
 *
 ...
 * 5. resize 원본 width/height 불변 — 별도 인스턴스에서 원본 치수 재확인
 */
class VipsPropertyTest : AbstractJVipsTest() {
    ...
    fun `resize 연산 후 원본 이미지의 치수는 변경되지 않는다`(fixturePath: String) {
        val bytes = VipsTestFixtures.loadFixture(fixturePath)
        vipsImageOf(bytes).use { original ->
