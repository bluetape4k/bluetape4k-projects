...
class GeoExamples: AbstractRedissonCoroutineTest() {
    ...
    fun `RGeo example`() = runSuspendIO {
        ...
        geo.addAsync(palermo, catania).awaitUntil() shouldBeEqualTo 2L

        val dist = geo.distAsync("Palermo", "Catania", GeoUnit.METERS).awaitUntil()
        val pos = geo.posAsync("Palermo", "Catania").awaitUntil()
        ...
        val cities = geo.searchAsync(fromLocation).awaitUntil()
        ...
        val allNearCities = geo.searchAsync(fromPalermo).awaitUntil()
        ...
        val citiesWithDistance: MutableMap<String, Double> =
            geo.searchWithDistanceAsync(searchArgsFromLocation).awaitUntil()
        ...
        val allNearCitiesDistance: MutableMap<String, Double> =
            geo.searchWithDistanceAsync(searchArgsFromPalermo).awaitUntil()
        ...
        val citiesWithPosition: MutableMap<String, GeoPosition> =
            geo.searchWithPositionAsync(searchArgsFromLocation).awaitUntil()
        ...
        val allNearCitiesPosition: MutableMap<String, GeoPosition> =
            geo.searchWithPositionAsync(searchArgsFromPalermo).awaitUntil()
        allNearCitiesPosition.forEach { (city, position) ->
            log.debug { "city=$city, position=$position" }
        }

        geo.deleteAsync().awaitUntil().shouldBeTrue()
    }
}
