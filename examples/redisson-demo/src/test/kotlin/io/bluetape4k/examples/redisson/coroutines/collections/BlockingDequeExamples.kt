...
class BlockingDequeExamples: AbstractRedissonCoroutineTest() {
    ...
    fun `blocking deque 를 사용`() = runSuspendIO {
        ...
        deque.putLastAsync("1").awaitUntil()
        deque.putLastAsync("2").awaitUntil()
        deque.putLastAsync("3").awaitUntil()
        deque.putLastAsync("4").awaitUntil()

        deque.containsAsync("1").awaitUntil().shouldBeTrue()

        // 첫번째 요소를 조회한다. (제거하지 않는다)
        deque.peekFirstAsync().awaitUntil() shouldBeEqualTo "1"
        // 첫번째 요소를 가져오고, queue에서는 제거한다 (첫번째 요소가 없으면 들어올 때까지 대기힌다.)
        deque.pollFirstAsync().awaitUntil() shouldBeEqualTo "1"
    ...
    fun `deque operation in coroutines`() = runSuspendIO {
        ...
        val retrieveJob = scope.launch {
            deque.pollAsync(10, TimeUnit.SECONDS).awaitUntil() shouldBeEqualTo 1
            yield()

            deque.takeAsync().awaitUntil() shouldBeEqualTo 2
            yield()

            // deque의 마지막 요소를 deque2의 첫번째 요소로 이동
            deque.pollLastAndOfferFirstToAsync(deque2.name, 5, TimeUnit.SECONDS).awaitUntil() shouldBeEqualTo 3
            yield()

            // deque의 마지막 요소를 deque2의 첫번째 요소로 이동
            deque.takeLastAndOfferFirstToAsync(deque2.name).awaitUntil() shouldBeEqualTo 4
            yield()
        }
        ...
        deque2.takeFirstAsync().awaitUntil() shouldBeEqualTo 3

        deque.putLastAsync(4).awaitUntil()
        delay(10.milliseconds)

        deque2.takeFirstAsync().awaitUntil() shouldBeEqualTo 4
