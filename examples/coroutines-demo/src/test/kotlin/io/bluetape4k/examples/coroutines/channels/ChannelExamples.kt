 ...
 *
 * - 기본 send/receive
 * - `for` 루프 및 `consumeEach`를 통한 수신
 * - `produce` 빌더를 이용한 채널 생성
 * - 채널 버퍼 유형: Unlimited, Buffered, Rendezvous, Conflated
 * - `onBufferOverflow` 옵션: SUSPEND, DROP_OLDEST, DROP_LATEST
 * - Fan-out (복수 Consumer 분배 수신) / Fan-in (복수 Producer → 단일 Channel)
 * - Pipeline 패턴 (채널 체이닝)
 */
class ChannelExamples {
    ...
    fun `buffer type - buffered`() = runTest {
        ...
        // send한 요소가 capqcity=3 까지 버퍼링 된다
    ...
    fun `buffer type - conflated`() = runTest {
        ...
        // Channel.CONFLATED는 send한 요소 중 가장 최신 것만 남기고 나머지는 버려버립니다. (0,2,4 는 버리고, 6 을 받습니다)
    ...
    fun `onBufferOverflow options`() = runTest {
        ...
        // Send 한 요소 중에 overflow 된 것은 버리고, 6, 8 만 남는다.
        advanceTimeBy(1000.milliseconds)
        val received = mutableListOf<Int>()
        for (element in channel) {
            received.add(element)
    ...
    inner class Pipeline {
        ...
        private fun CoroutineScope.numbers(times: Int = 5) =
            produce(capacity = 4) {
                repeat(times) { num ->
        ...
        private fun CoroutineScope.square(numbers: ReceiveChannel<Int>): ReceiveChannel<Int> =
            produce(capacity = 2) {
                for (num in numbers) {
