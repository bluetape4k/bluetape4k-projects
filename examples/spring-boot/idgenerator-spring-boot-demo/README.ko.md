...
이 예제는 `bluetape4k-idgenerators`를 Spring Boot REST 애플리케이션으로 노출하는 방법을 보여줍니다. Ktor 예제는 #419에서 따로 다룹니다.
...
`IdGeneratorConfiguration`은 각 generator를 Spring Bean으로 등록합니다. `IdGeneratorRegistry`는 REST 타입 이름을 실제 generator에 매핑하고, `IdGeneratorService`는 batch size를 검증합니다.
...
테스트는 Spring Boot 애플리케이션을 로드하고 모든 REST endpoint를 검증합니다. 또한 `SuspendedJobTester`로 UUID v7과 Snowflake 병렬 요청이 중복 없는 ID를 반환함을 증명합니다.
