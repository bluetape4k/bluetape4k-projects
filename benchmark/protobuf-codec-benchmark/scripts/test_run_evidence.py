...
"""
    ...
    def test_run_captures_validator_schema_and_rejects_run_directory_collision(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            pinned_ref = "<PINNED_JAR_SHA256:{}>".format(environment["benchmark_jar_sha256"])
            self.assertNotIn("executed_jar_path", environment)
            self.assertNotIn("benchmark_jar_path", environment)
            self.assertEqual(pinned_ref, environment["executed_jar_ref"])
            self.assertEqual(pinned_ref, environment["benchmark_jar_ref"])
            self.assertEqual(pinned_ref, environment["jmh_argv"][2])
            self.assertEqual("jmh.json", environment["jmh_argv"][environment["jmh_argv"].index("-rff") + 1])
            serialized_environment = runner.canonical_json_bytes(environment)
            for forbidden in (str(root).encode(), b"/Users/", b"/private/tmp/", b"/build/"):
                with self.subTest(forbidden=forbidden):
