...
class OrderProcessingWorkflowTest {
    ...
    private fun validateOrder(): Work = Work("order-validate") { ctx ->
    ...
    private fun checkInventory(available: Boolean = true): Work = Work("inventory-check") { ctx ->
        Thread.sleep(10)
    ...
    private fun checkFraud(passed: Boolean = true): Work = Work("fraud-check") { ctx ->
        Thread.sleep(15)
        ctx["fraud.ok"] = passed
    ...
    private fun validateCoupon(): Work = Work("coupon-validate") { ctx ->
        Thread.sleep(5)
     ...
     */
    private fun requestPayment(successOnAttempt: Int = 1): Work {
        var attempt = 0
     ...
     */
    private fun pollPgApproval(approvedOnPoll: Int = 2): Work {
        var poll = 0
    ...
    private fun confirmOrder(): Work = Work("order-confirm") { ctx ->
        ctx["order.status"] = "CONFIRMED"
        log.debug { "⑤ 주문 확정 완료 (txId=${ctx.get<String>("pg.txId")})" }
        WorkReport.success(ctx)
    }

    private fun cancelOrder(): Work = Work("order-cancel") { ctx ->
    ...
    fun `재고 부족 - ABORTED 즉시 전파, 결제 단계 미실행`() {
        ...
        (ctx.get<String>("pg.txId") == null).shouldBeTrue()   // 결제 미실행
    ...
    fun `PG 승인 타임아웃 - 5회 폴링 후 미승인, 주문 취소`() {
        // pgApprovedOnPoll=99 → maxIterations=5 내에 승인 안 됨 → pg.approved = false
        val ctx = workContext(
            "order.id" to "ORD-006",
