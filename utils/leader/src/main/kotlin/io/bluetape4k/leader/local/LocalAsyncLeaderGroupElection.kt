...
/**
 * [Semaphore]를 이용한 로컬(단일 JVM) 복수 리더 비동기 선출 구현체입니다.
 *
 * ## 동작
 ...
 * - `action`이 `() -> CompletableFuture<T>` 비동기 람다로, [CompletableFuture]를 반환합니다.
 * - [Executor]를 지정할 수 있어 비동기 실행 스레드를 제어할 수 있습니다.
...
class LocalAsyncLeaderGroupElection private constructor(
    ...
    companion object : KLogging() {
        ...
        operator fun invoke(maxLeaders: Int = 2): AsyncLeaderGroupElection {
            ...
            return LocalAsyncLeaderGroupElection(maxLeaders)
        }
    }
    ...
    /**
     * [lockName]의 [Semaphore] 슬롯을 [executor]에서 획득하고 비동기 [action]을 실행합니다.
     *
     * - 슬롯이 가득 찬 경우 빈 슬롯이 생길 때까지 [executor] 스레드가 블로킹됩니다.
