...
class BoundingBoxExtensionsTest {
    ...
    companion object: KLogging() {
        private const val DELTA = 1.0e-9
    }
...
//        }
        val bytes = BinarySerializers.FastFory.serialize(box)
        val restored = BinarySerializers.FastFory.deserialize<BoundingBox>(bytes).shouldNotBeNull()

        restored.isIntersection180Meridian.shouldBeTrue()
        restored.contains(wgs84PointOf(0.0, 175.0)).shouldBeTrue()
        BoundingBox::class.lookup().serialVersionUID shouldBeEqualTo -1275515777007294183L
    }
    ...
    fun `잘못된 직접 변경은 공간 연산 진입 시 거부한다`() {
        ...
        assertFailsWith<IllegalArgumentException> {
            box.contains(wgs84PointOf(0.0, 0.0))
        }
    }
    ...
    fun `직접 생성과 copy에서도 좌표 불변식을 검증한다`() {
        ...
        val box = boundingBoxOf(-10.0, 10.0, -20.0, 20.0)
        assertFailsWith<IllegalArgumentException> {
            box.copy(eastLongitude = 181.0)
        }
        assertFailsWith<IllegalArgumentException> {
            box.copy(northLatitude = Double.NaN)
        }
    }
    ...
    fun `BoundingBox equals and hashCode`() {
        ...
        bbox1 shouldBeEqualTo bbox2
        bbox1.hashCode() shouldBeEqualTo bbox2.hashCode()
    }
    ...
    fun `WGS84Point moveInDirection moves point`() {
        ...
        moved.longitude shouldBeGreaterThan start.longitude
    }
