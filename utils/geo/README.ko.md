...
BoundingBox는 현재 좌표로 자오선 교차 여부를 계산합니다. 생성자와 copy는 좌표 범위를 검증합니다. 공개 변경 가능 프로퍼티는 유지하며 직접 변경 후 공간 연산과 직렬화에서 좌표를 검증합니다. setter 대입 자체의 원자적 검증이나 동시 변경의 안전성을 보장하지 않습니다.
