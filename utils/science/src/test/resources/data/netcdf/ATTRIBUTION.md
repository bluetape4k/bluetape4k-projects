...
Unidata 의 NetCDF examples 페이지에는 명시적 라이선스 문구가 없습니다. Unidata 와 NCAR 가 공개 교육·연구 목적으로 배포하는 샘플 데이터로 일반적으로 자유롭게 재배포·재사용이 가능하다고 알려져 있으나 공식 라이선스 텍스트는 부재하므로 본 프로젝트는 다음 원칙을 따릅니다:
...
본 파일은 원저작권자 (Unidata / NCAR) 가 변경 / 삭제 요청 시 즉시 본 디렉토리에서 제거됩니다. 또한 본 attribution 의 정확성은 다운로드 시점 기준이며, 원본 페이지의 라이선스 정책이 변경된 경우 사용자가 별도로 확인해야 합니다.
