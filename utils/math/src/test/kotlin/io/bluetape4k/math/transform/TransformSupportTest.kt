...
class TransformSupportTest {
    ...
    fun `DoubleArray를 스케일링할 수 있다`() {
        ...
        result[0].shouldBeNear(2.0, EPSILON)
        result[1].shouldBeNear(4.0, EPSILON)
        result[2].shouldBeNear(6.0, EPSILON)
    }
    ...
    fun `DoubleArray를 0으로 스케일링하면 모두 0이 된다`() {
        ...
        result[2].shouldBeNear(0.0, EPSILON)
    }
    ...
    fun `복소수 배열을 스케일링할 수 있다`() {
        ...
        val result = data.scale(2.0)
        result[0].real.shouldBeNear(2.0, EPSILON)
        result[0].imaginary.shouldBeNear(0.0, EPSILON)
        result[1].real.shouldBeNear(0.0, EPSILON)
        result[1].imaginary.shouldBeNear(2.0, EPSILON)
    }
    ...
    fun `복소수 배열을 실수부와 허수부 2차원 배열로 변환할 수 있다`() {
        ...
        result[0][0].shouldBeNear(1.0, EPSILON)
        result[0][1].shouldBeNear(3.0, EPSILON)
        result[1][0].shouldBeNear(2.0, EPSILON)
        result[1][1].shouldBeNear(4.0, EPSILON)
    }
    ...
    fun `2차원 실수 배열을 복소수 배열로 변환할 수 있다`() {
        ...
        result[0].real.shouldBeNear(1.0, EPSILON)
        result[0].imaginary.shouldBeNear(2.0, EPSILON)
        result[1].real.shouldBeNear(3.0, EPSILON)
        result[1].imaginary.shouldBeNear(4.0, EPSILON)
    }
    ...
    fun `toRealImaginaryArray 후 toComplexArray로 왕복 변환된다`() {
        ...
        roundTrip[0].real.shouldBeNear(original[0].real, EPSILON)
        roundTrip[0].imaginary.shouldBeNear(original[0].imaginary, EPSILON)
        roundTrip[1].real.shouldBeNear(original[1].real, EPSILON)
        roundTrip[1].imaginary.shouldBeNear(original[1].imaginary, EPSILON)
    }
}
