...
AWS SDK services are declared as `compileOnly` dependencies, so you need to add the runtime dependencies for the services you use.
...
// Query DynamoDB using a suspend function
...
flowchart LR
    ...
    ASYNC -->|"coroutine wrapping<br/>(.await())"| CORO
```
...
```mermaid
flowchart TD
    MOD["bluetape4k-aws<br/>(unified integration module)"]
