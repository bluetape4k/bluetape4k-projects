package io.bluetape4k.exposed.core.jasypt

import io.bluetape4k.logging.KLogging
import io.bluetape4k.logging.debug
import org.jetbrains.exposed.v1.core.ColumnTransformer
import org.jetbrains.exposed.v1.core.ColumnWithTransform
import org.jetbrains.exposed.v1.core.VarCharColumnType

/**
 * 문자열 값을 암호화해서 `VARCHAR` 컬럼에 저장하는 변환 컬럼 타입입니다.
 *
 * ## 동작/계약
 * - DB 저장 시 [StringJasyptEncryptionTransformer.unwrap]로 암호화하고, 조회 시 `wrap`으로 복호화합니다.
 * - 수신 값을 mutate하지 않고 암호화/복호화된 새 문자열을 반환합니다.
 * - 컬럼 길이 검증은 생성 전에 호출되는 `jasyptVarChar(...).requirePositiveNumber(...)` 경로에서 수행됩니다.
 * - 테스트처럼 동일 문자열 입력은 동일 암호문으로 변환되어 인덱스/조건 검색에 사용할 수 있습니다.
 *
 * ```kotlin
 * val transformer = StringJasyptEncryptionTransformer(Encryptors.AES)
 * val source = "deterministic-source"
 * val encrypted1 = transformer.unwrap(source)
 * val encrypted2 = transformer.unwrap(source)
 * // encrypted1 == encrypted2
 * ```
 *
 * @param encryptor 문자열 암/복호화를 수행할 암호화기입니다.
 * @param colLength 암호문을 저장할 컬럼 길이입니다.
 */
class JasyptVarCharColumnType(
    private val encryptor: io.bluetape4k.crypto.encrypt.Encryptor,
    colLength: Int,
): ColumnWithTransform<String, String>(
    VarCharColumnType(colLength),
    StringJasyptEncryptionTransformer(encryptor)
)

/**
 * 문자열 컬럼의 저장/조회 경계에서 Jasypt 암복호화를 수행하는 transformer입니다.
 *
 * ## 동작/계약
 * - [unwrap]은 평문 문자열을 암호문 문자열로 변환해 DB 저장 값으로 사용합니다.
 * - [wrap]은 DB에서 읽은 암호문 문자열을 복호화해 엔티티/ResultRow 값으로 제공합니다.
 * - round-trip(`wrap(unwrap(x))`)은 테스트 기준으로 입력 문자열과 동일합니다.
 *
 * ```kotlin
 * val transformer = StringJasyptEncryptionTransformer(Encryptors.AES)
 * val source = "jasypt-string-source"
 * val restored = transformer.wrap(transformer.unwrap(source))
 * // restored == source
 * ```
 *
 * @param encryptor 문자열 암/복호화 구현체입니다.
 */
class StringJasyptEncryptionTransformer(
    private val encryptor: io.bluetape4k.crypto.encrypt.Encryptor,
): ColumnTransformer<String, String> {

    companion object: KLogging()

    /**
     * 평문 문자열을 암호문 문자열로 변환합니다.
     *
     * ## 동작/계약
     * - 입력 [value]를 [encryptor]로 암호화합니다.
     * - 입력 문자열 자체는 변경하지 않고, 암호문 문자열을 새로 반환합니다.
     *
     * ```kotlin
     * val transformer = StringJasyptEncryptionTransformer(Encryptors.AES)
     * val encrypted = transformer.unwrap("jasypt-string-source")
     * // encrypted != "jasypt-string-source"
     * ```
     *
     * @param value 암호화할 평문 문자열입니다.
     */
    override fun unwrap(value: String): String {
        log.debug { "Encrypting value=$value" }
        return encryptor.encrypt(value).apply {
            log.debug { "Encrypted value=$this" }
        }
    }

    /**
     * 암호문 문자열을 복호화해 평문 문자열로 변환합니다.
     *
     * ## 동작/계약
     * - 입력 [value]를 [encryptor]로 복호화합니다.
     * - 암호문 형식이 아니거나 키가 맞지 않으면 encryptor 구현 예외가 전파됩니다.
     *
     * ```kotlin
     * val transformer = StringJasyptEncryptionTransformer(Encryptors.AES)
     * val source = "jasypt-string-source"
     * val restored = transformer.wrap(transformer.unwrap(source))
     * // restored == source
     * ```
     *
     * @param value 복호화할 암호문 문자열입니다.
     */
    override fun wrap(value: String): String {
        log.debug { "Decrypting value=$value" }
        return encryptor.decrypt(value).apply {
            log.debug { "Decrypted value=$this" }
        }
    }
}
