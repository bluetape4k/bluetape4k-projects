...
import io.bluetape4k.exposed.ignite3.AbstractIgnite3Test
import io.bluetape4k.exposed.ignite3.repository.UserSchema.UserTable
import io.bluetape4k.exposed.ignite3.repository.UserSchema.getExistingId
import io.bluetape4k.exposed.ignite3.repository.UserSchema.getExistingIds
import io.bluetape4k.exposed.ignite3.repository.UserSchema.getNonExistentId
import io.bluetape4k.exposed.ignite3.repository.UserSchema.withUserTable
...
/**
 * Apache Ignite 3.x 기반 [UserIgnite3CacheRepository]의 Read-Through 캐시 동작을 검증하는 테스트입니다.
 *
 ...
 *
 * Ignite 3.x 캐시 테이블은 아래 SQL로 생성되어야 합니다:
 * ```sql
 ...
 * ```
 */
class Ignite3CacheReadThroughTest: AbstractIgnite3Test() {
    ...
    private val config = IgniteNearCacheConfig(tableName = "TEST_USERS")

    private val repository by lazy {
        UserIgnite3CacheRepository(igniteClient, config)
    }
    ...
    fun `get으로 ID 조회 시 DB에서 읽어 Ignite3 캐시에 저장 후 반환한다`(testDB: TestDB) {
    ...
    @ParameterizedTest
    @MethodSource("enableDialects")
    fun `findAll로 전체 엔티티를 DB에서 조회하고 Ignite3 캐시에 저장한다`(testDB: TestDB) {
        withUserTable(testDB) {
