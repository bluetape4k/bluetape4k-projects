...
/**
 * Exposed와 Apache Ignite 3.x [IgniteNearCache]를 활용하여 데이터를 캐싱하는 Repository 인터페이스입니다.
 *
 * Ignite 3.x 오픈소스는 클라이언트 측 Near Cache를 내장하지 않으므로,
 * Caffeine(Front) + [org.apache.ignite.table.KeyValueView](Back) 2-Tier 구조의
...
interface IgniteCacheRepository<T: HasIdentifier<ID>, ID: Any> {
    ...
    /** 캐시 이름 (Ignite 3.x 테이블 이름) */
    val cacheName: String

    /** Exposed 엔티티 테이블 */
    val entityTable: IdTable<ID>

    /** Ignite 3.x 기반 NearCache (Caffeine Front + KeyValueView Back) */
    ...
    /**
     * 엔티티를 NearCache에 저장합니다 (Front Cache + Ignite 3.x Back Cache).
     *
     ...
     * Back Cache(Ignite 3.x KeyValueView)는 변경되지 않습니다.
     */
