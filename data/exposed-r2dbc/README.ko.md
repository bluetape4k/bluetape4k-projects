...
`bluetape4k-exposed-r2dbc`는 `bluetape4k-r2dbc`를 노출하므로 Exposed R2DBC 애플리케이션에서도
`R2dbcPoolConfig.highThroughput()`을 재사용하고 생성된 `ConnectionPool`을 `R2dbcDatabase`에 전달할 수 있습니다.
...
풀 benchmark는 `bluetape4k-r2dbc`에 있습니다. `./gradlew :bluetape4k-r2dbc:benchmarkPoolConfig`,
...
`./gradlew :bluetape4k-r2dbc:benchmarkH2PoolContention`으로 실행합니다.

로컬 H2/PostgreSQL/MySQL8 실측에서는 순수 acquire/close 처리량은 드라이버마다 달랐지만, 요청이 커넥션을
