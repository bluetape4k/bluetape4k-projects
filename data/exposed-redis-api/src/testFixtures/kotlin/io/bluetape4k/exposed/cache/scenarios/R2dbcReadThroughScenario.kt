...
/**
 * [R2dbcCacheTestScenario] 기반 Read-Through 캐시 suspend 시나리오입니다.
 *
 * R2DBC 환경에서 캐시 미스 시 DB에서 읽어 캐시에 저장하는 패턴을 검증합니다.
 *
 * @param ID 엔티티의 식별자 타입
 * @param E 엔티티 타입
 */
interface R2dbcReadThroughScenario<ID: Any, E: Serializable>: R2dbcCacheTestScenario<ID, E> {
    ...
    fun `invalidate - Read-Through에서 캐시 invalidate는 DB에 영향을 주지 않는다`(testDB: TestDB) = runTest {
        withR2dbcEntityTable(testDB) {
            ...
            if (cacheWriteMode == CacheWriteMode.READ_ONLY) {
