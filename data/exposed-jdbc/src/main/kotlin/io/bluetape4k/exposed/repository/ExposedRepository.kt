 ...
 */
@Deprecated(
    message = "use JdbcRepository instead.",
    ReplaceWith("JdbcRepository"),
...
interface ExposedRepository<T: HasIdentifier<ID>, ID: Any> {
     ...
     * @return 엔티티 [T] 또는 null
     */
    fun <V> findByFieldOrNull(field: Column<V>, value: V): T? =
        table.selectAll()
            .where { field eq value }
            .firstOrNull()
            ?.toEntity()
     ...
     * @return 엔티티 목록
     */
    fun findAllByIds(ids: Iterable<ID>): List<T> =
        table.selectAll()
     ...
     * @param sortOrder 정렬 순서
     * @param predicate 조건
     * @return 페이징 결과 [io.bluetape4k.exposed.core.ExposedPage]
     */
    fun findPage(
        pageNumber: Int,
        pageSize: Int,
