...
class SuspendedQueryTest: AbstractExposedTest() {
    ...
    fun `코루틴 환경에서 AutoInc ID를 가진 테이블에서 Batch 로 읽어온다`(testDB: TestDB) =
        runSuspendIO {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `코루틴 환경에서 ID가 SnowflakeID의 Long 수형의 테이블에서 Batch 로 읽어온다`(testDB: TestDB) =
        runSuspendIO {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `Batch 로 읽기에서 Int-Long 이 아닌 id 컬럼은 예외를 던진다`(testDB: TestDB) =
        runSuspendIO {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `batchSize 가 0 이하이면 예외를 던진다`(testDB: TestDB) =
        runSuspendIO {
            withTablesSuspending(testDB, ProductTable) {
                assertFailsWith<IllegalArgumentException> {
                    ProductTable
                        .select(ProductTable.id)
                        .fetchBatchedResultFlow(0)
                        .toList()
                }
                assertFailsWith<IllegalArgumentException> {
                    ProductTable
