...
/**
 * [SuspendedJdbcCacheRepository] 인터페이스 단위 테스트.
 *
...
class SuspendedJdbcCacheRepositoryTest {
    ...
    fun `DEFAULT_BATCH_SIZE 상수는 500이다`() {
        SuspendedJdbcCacheRepository.DEFAULT_BATCH_SIZE shouldBeEqualTo 500
    }

    @Test
    fun `Companion 객체에 접근할 수 있다`() {
        val companion = SuspendedJdbcCacheRepository.Companion
    ...
    fun `close() 기본 구현은 예외 없이 완료된다`() {
        val repo = MinimalSuspendedJdbcCacheRepository()
        repo.close()
    }
