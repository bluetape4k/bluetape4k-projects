...
/**
 * [SuspendJdbcRedisRepository] 인터페이스 단위 테스트.
 *
 * invalidateByPattern의 기본 count 파라미터($default 메서드 경로)를 커버합니다.
 */
class SuspendJdbcRedisRepositoryTest {
    ...
    @Test
    fun `invalidateByPattern을 count 없이 호출하면 DEFAULT_BATCH_SIZE가 사용된다`() = runSuspendIO {
        var capturedCount = -1
        val repo: SuspendJdbcRedisRepository<Long, DummyEntity> = object : SuspendJdbcRedisRepository<Long, DummyEntity> {
