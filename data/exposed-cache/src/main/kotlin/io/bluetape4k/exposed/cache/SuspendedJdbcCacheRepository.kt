...
import io.bluetape4k.exposed.cache.SuspendedJdbcCacheRepository.Companion.DEFAULT_BATCH_SIZE
...
/**
 * Exposed JDBC + Redis 캐시 기반 suspend 저장소의 공통 인터페이스입니다.
 *
 * [JdbcCacheRepository]와 동일한 기능을 제공하되,
 * 모든 메서드가 `suspend` 함수로 정의되어 코루틴 환경에서 사용할 수 있습니다.
 * 내부적으로 `suspendedTransactionAsync`를 사용하여 JDBC를 suspend로 래핑합니다.
 *
...
interface SuspendedJdbcCacheRepository<ID: Any, E: Serializable>: Closeable {
     ...
     *
     * JDBC suspend 저장소에서는 non-suspend로 유지합니다.
     * (suspendedTransactionAsync 내부에서 호출되므로)
     *
     * @return 변환된 엔티티
     */
     ...
     */
    suspend fun putAll(entities: Map<ID, E>, batchSize: Int = DEFAULT_BATCH_SIZE)
