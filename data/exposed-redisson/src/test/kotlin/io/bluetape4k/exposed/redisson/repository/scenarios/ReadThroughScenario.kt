...
abstract class ReadThroughScenario<T: HasIdentifier<ID>, ID: Any>: AbstractRedissonTest() {
     ...
     * 테스트에 사용할 캐시 저장소
     */
    abstract val repository: ExposedCacheRepository<T, ID>

    /**
     * 테스트에 사용할 테이블을 설정하고 테스트 로직을 실행하는 함수
     */
    abstract fun withEntityTable(testDB: TestDB, statement: Transaction.() -> Unit)

    /**
     * 테스트에서 사용할 샘플 ID를 반환합니다
     */
    abstract fun getExistingId(): ID

    abstract fun getExistingIds(): List<ID>

    /**
     * 테스트에서 사용할 존재하지 않는 ID를 반환합니다
     */
    abstract fun getNonExistentId(): ID
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    open fun `invalidte(id) - 캐시 invalidate`(testDB: TestDB) {
        withEntityTable(testDB) {
            val id = getExistingId()
