 ...
 * - [toScalar]가 `true`이면 scalar/text, `false`이면 JSON 객체 모드로 렌더링됩니다.
 * - [path]는 가변 인자로 전달되며 벤더별 다중 경로 지원 여부는 다를 수 있습니다.
 *
 * ```kotlin
 ...
 *   DB에서 추출된 JSON 경로 값이 `null`로 역직렬화되는 것은 경로 불일치나 데이터 정합성 오류를 나타내기 때문입니다.
 ...
 * ```
 *
 * @param path JSON 경로 목록입니다. 비워두면 벤더 기본 루트 규칙을 따릅니다.
 * @param toScalar `true`면 scalar/text, `false`면 JSON 객체 모드로 추출합니다.
 * @param serializer 추출 결과를 `T`로 변환할 때 사용할 Jackson3 serializer입니다.
 */
