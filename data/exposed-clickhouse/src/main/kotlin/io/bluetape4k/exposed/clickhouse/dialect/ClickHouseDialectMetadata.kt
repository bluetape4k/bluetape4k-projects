...
/**
 * ClickHouse용 Exposed 메타데이터 어댑터입니다.
 *
 * 기본 메타데이터 해석은 [PostgreSQLDialectMetadata]를 재사용하되,
 * ClickHouse JDBC가 지원하지 않는 메타데이터 조회는 안전하게 우회합니다.
 */
class ClickHouseDialectMetadata: PostgreSQLDialectMetadata() {

    /**
     * ClickHouse JDBC는 [java.sql.DatabaseMetaData.getImportedKeys]를 통한
     * FK 메타데이터 조회를 지원하지 않습니다.
     * 따라서 FK 제약 조건 캐시 적재를 생략하고, 관련 조회는 빈 결과로 처리합니다.
     */
    override fun fillConstraintCacheForTables(tables: List<Table>) {
        // no-op: ClickHouse JDBC는 FK 메타데이터 미지원
    }
}
