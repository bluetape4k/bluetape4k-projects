...
class MongoDatabaseExtensionsTest : AbstractMongoTest() {
    ...
    @Test
    fun `getCollectionOf reified 타입으로 컬렉션 획득`() = runTest(timeout = 30.seconds) {
        val collection = database.getCollectionOf<Document>("test_collection")
        collection.shouldNotBeNull()
    }

    @Test
    fun `listCollectionNamesList 컬렉션 이름 목록 반환`() = runTest(timeout = 30.seconds) {
        // 테스트용 컬렉션 생성
