...
class MovieCoroutineRepository: AbstractExposedCoroutineRepository<MovieEntity, Long>(MovieTable) {
    ...
    suspend fun searchMovies(params: Map<String, String?>): List<MovieEntity> {
    ...
    suspend fun save(movieDto: MovieDTO): MovieEntity {
    ...
    fun getAllMoviesWithActors(): Flow<MovieWithActorDTO> {
        ...
        return movies.values.flatten().asFlow()
    }
    ...
    fun findMoviesWithActingProducers(): Flow<MovieWithProducingActorDTO> {
        ...
        return query.asFlow().map { it.toMovieWithProducingActorDTO() }
    }
