...
/**
 * 인덱스 컬럼 값을 Jackson으로 역직렬화해 non-null 결과를 반환합니다.
 *
 * ## 동작/계약
...
/**
 * 이름 컬럼 값을 Jackson으로 역직렬화해 non-null 결과를 반환합니다.
 *
 * ## 동작/계약
 * - 내부적으로 [getJacksonOrNull]을 호출하고 `null`이면 `error(...)`를 발생시킵니다.
...
/**
 * 인덱스 컬럼 값을 Jackson으로 역직렬화하고, 원본 값이 `null`이면 `null`을 반환합니다.
 *
 * ## 동작/계약
...
/**
 * 이름 컬럼 값을 Jackson으로 역직렬화하고, 원본 값이 `null`이면 `null`을 반환합니다.
 *
 * ## 동작/계약
