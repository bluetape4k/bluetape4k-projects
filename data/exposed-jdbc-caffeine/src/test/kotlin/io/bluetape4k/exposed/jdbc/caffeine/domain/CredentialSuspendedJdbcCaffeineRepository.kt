...
/**
 * JDBC Caffeine suspend 통합 테스트용 [CredentialRecord] 레포지토리 구체 구현체.
 *
 * Client-generated UUID ID ([CredentialTable])를 사용합니다.
 */
class CredentialSuspendedJdbcCaffeineRepository(
    config: LocalCacheConfig = LocalCacheConfig.WRITE_THROUGH,
): AbstractSuspendedJdbcCaffeineRepository<UUID, CredentialRecord>(config) {
