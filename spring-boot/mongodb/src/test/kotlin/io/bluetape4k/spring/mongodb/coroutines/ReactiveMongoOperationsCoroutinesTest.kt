...
class ReactiveMongoOperationsCoroutinesTest: AbstractReactiveMongoCoroutineTest() {
    ...
    fun `aggregateAsFlow - 도시별 사용자 수를 집계한다`() = runTest {
        ...
        val cityCount = results.find { it.id == "Seoul" }
        cityCount.shouldNotBeNull()
        cityCount.count shouldBeEqualTo 3
    }
    ...
    fun `collectionExistsSuspending - 컬렉션 존재 여부를 확인한다`() = runTest {
        mongoOperations.collectionExistsSuspending("test_users").shouldBeTrue()
    }
    ...
    fun `dropCollectionSuspending(name) - 이름으로 컬렉션을 삭제한다`() = runTest {
        ...
        mongoOperations.collectionExistsSuspending(collName).shouldBeFalse()
    }
}
