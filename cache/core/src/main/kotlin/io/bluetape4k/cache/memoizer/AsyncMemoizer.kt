 ...
 *
 * ```kotlin
 * val memo: AsyncMemoizer<String, Int> =
 *   ({ key -> CompletableFuture.completedFuture(key.length) }).asyncMemoizer()
 * val size = memo("abcd").join()
 * // size == 4
 * ```
 */
interface AsyncMemoizer<in T: Any, R: Any>: (T) -> CompletableFuture<R> {
