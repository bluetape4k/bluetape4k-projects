...
class ResilientSuspendNearJCacheTest {
    ...
    private val backCache = CaffeineSuspendJCache<String, String> {
        maximumSize(10_000)
    ...
    @BeforeEach
    fun createCache() {
        cache = ResilientSuspendNearJCache(
            backCache = backCache,
    ...
    @Test
    fun `put - full write channel fails before front update`() = runSuspendIO {
    ...
    @Test
    fun `put - retry exhaustion invalidates uncommitted front value`() = runSuspendIO {
    ...
    @Test
    fun `get - concurrent put prevents stale read-through population`() = runSuspendIO {
    ...
    @Test
    fun `get - concurrent replace prevents stale read-through population`() = runSuspendIO {
        val readStarted = CountDownLatch(1)
        val releaseRead = CountDownLatch(1)
        val staleBackCache = mockk<SuspendJCache<String, String>>(relaxed = true)
        coEvery { staleBackCache.get("shared") } coAnswers {
    ...
    @Test
    fun `get - CancellationException은 fallback하지 않고 재전파한다`() = runSuspendIO {
        val failingCache = resilientCacheWithFailingBackCache { backCache ->
    ...
    @Test
    fun `putAll - snapshots mutable entries before enqueue`() = runSuspendIO {
    ...
    @Test
    fun `removeAll - snapshots mutable keys before enqueue`() = runSuspendIO {
        val firstPutStarted = CountDownLatch(1)
        val releaseFirstPut = CountDownLatch(1)
    ...
    @Test
    fun `SuspendedJobTester - stale completions preserve latest accepted state`() = runSuspendIO {
    ...
    @Test
    fun `getAll - CancellationException은 fallback하지 않고 재전파한다`() = runSuspendIO {
        val failingCache = resilientCacheWithFailingBackCache { backCache ->
    ...
    @Test
    fun `remove - retry exhaustion keeps tombstone and blocks stale back reads`() = runSuspendIO {
    ...
    @Test
    fun `removeAll - retry exhaustion keeps tombstones and blocks stale back reads`() = runSuspendIO {
        val removeAttempts = AtomicInteger()
        val failingCache = resilientCacheWithFailingBackCache { backCache ->
    ...
    @Test
    fun `containsKey - CancellationException은 false fallback하지 않고 재전파한다`() = runSuspendIO {
        val failingCache = resilientCacheWithFailingBackCache { backCache ->
    ...
    @Test
    fun `putIfAbsent - queued remove preserves mutation order`() = runSuspendIO {
    ...
    @Test
    fun `putIfAbsent - queued clear preserves mutation order`() = runSuspendIO {
    ...
    @Test
    fun `putIfAbsent - newer clear supersedes pending put`() = runSuspendIO {
    ...
    @Test
    fun `replace - queued remove prevents replacement`() = runSuspendIO {
        cache.put("key", "old")
        await atMost 5.seconds untilSuspending { backCache.get("key") == "old" }
    ...
    @Test
    fun `replace - containsKey CancellationException은 false fallback하지 않고 재전파한다`() = runSuspendIO {
        val failingCache = resilientCacheWithFailingBackCache { backCache ->
    ...
    @Test
    fun `clearAll - retry exhaustion keeps clear pending and blocks stale back reads`() = runSuspendIO {
    ...
    @Test
    fun `close - timeout completes pending write exceptionally`() = runSuspendIO {
    ...
    @Test
    fun `close drains queued write-behind commands`() = runSuspendIO {
        val closeDrainBackCache = CaffeineSuspendJCache<String, String> {
            maximumSize(10_000)
            expireAfterWrite(Duration.ofMinutes(30))
        }
