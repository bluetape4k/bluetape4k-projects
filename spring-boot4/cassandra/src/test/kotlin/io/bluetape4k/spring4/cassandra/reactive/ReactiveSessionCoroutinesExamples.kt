...
import io.bluetape4k.spring4.cassandra.executeSuspending
import io.bluetape4k.spring4.cassandra.prepareSuspending
...
class ReactiveSessionCoroutinesExamples(
    ...
    fun `execute cql in coroutines`() = runSuspendIO {
        ...
        val rrset: ReactiveResultSet = reactiveSession.executeSuspending(cql, 1212L)
    ...
    fun `execute cql with map in coroutines`() = runSuspendIO {
        ...
        val rrset: ReactiveResultSet = reactiveSession.executeSuspending(cql, mapOf("id" to 1212L))
        val rows = rrset.rows().asFlow().toList()
    ...
    fun `execute statement in coroutines`() = runSuspendIO {
        ...
        val rrset: ReactiveResultSet = reactiveSession.executeSuspending(bs)
        val rows = rrset.rows().asFlow().toList()
