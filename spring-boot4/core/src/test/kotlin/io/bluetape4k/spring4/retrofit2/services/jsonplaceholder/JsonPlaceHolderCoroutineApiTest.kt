package io.bluetape4k.spring4.retrofit2.services.jsonplaceholder
...
class JsonPlaceHolderCoroutineApiTest : AbstractJsonPlaceHolderApiTest() {
    ...
    @Test
    fun `get post by postId`(
        @RandomValue(type = Int::class, size = 3) postIds: List<Int>,
    ) = runSuspendIO {
        val deferreds =
            postIds.map {
    ...
    @Test
    fun `get users posts`(
        @RandomValue(type = Int::class, size = 3) userIds: List<Int>,
    ) = runSuspendIO {
        val deferreds =
            userIds.map {
    ...
    @Test
    fun `create new post`(
        @RandomValue(type = Post::class, size = 5) posts: List<Post>,
    ) = runSuspendIO {
        val tasks =
            posts.map { post ->
                async(Dispatchers.IO) {
    ...
    @Test
    fun `update existing post`(
        @RandomValue(type = Int::class, size = 5) postIds: List<Int>,
    ) = runSuspendIO {
        val deferreds =
            postIds.map {
                val postId = it.absoluteValue % 50 + 1
