...
fun Publisher<out DataBuffer>.write(outputStream: OutputStream): Flow<DataBuffer> =
    DataBufferUtils.write(this as Publisher<DataBuffer>, outputStream).asFlow()
...
fun Publisher<out DataBuffer>.write(channel: WritableByteChannel): Flow<DataBuffer> =
    DataBufferUtils.write(this as Publisher<DataBuffer>, channel).asFlow()
...
fun Publisher<out DataBuffer>.write(
    channel: AsynchronousFileChannel,
    position: Long = 0,
): Flow<DataBuffer> = DataBufferUtils.write(this, channel, position).asFlow()
...
suspend fun Publisher<out DataBuffer>.write(
    destination: Path,
...
fun Publisher<out DataBuffer>.takeUntilByteCount(maxByteCount: Long): Flow<DataBuffer> =
    DataBufferUtils.takeUntilByteCount(this, maxByteCount).asFlow()
...
fun Publisher<out DataBuffer>.skipUntilByteCount(maxByteCount: Long): Flow<DataBuffer> =
    DataBufferUtils.skipUntilByteCount(this, maxByteCount).asFlow()
 ...
 * ```
 */
fun DataBuffer.release(): Boolean = DataBufferUtils.release(this)
