...
import io.bluetape4k.spring4.mongodb.AbstractReactiveMongoCoroutineTest
import io.bluetape4k.spring4.mongodb.model.User
import io.bluetape4k.spring4.mongodb.query.andSet
import io.bluetape4k.spring4.mongodb.query.criteria
import io.bluetape4k.spring4.mongodb.query.eq
import io.bluetape4k.spring4.mongodb.query.gt
import io.bluetape4k.spring4.mongodb.query.paginate
import io.bluetape4k.spring4.mongodb.query.queryOf
import io.bluetape4k.spring4.mongodb.query.setTo
import io.bluetape4k.spring4.mongodb.query.sortAscBy
import io.bluetape4k.spring4.mongodb.query.sortDescBy
import io.bluetape4k.spring4.mongodb.query.toQuery
...
class ReactiveMongoOperationsCoroutinesTest : AbstractReactiveMongoCoroutineTest() {
    ...
    @Test
    fun `insertSuspending - 새 사용자를 삽입한다`() =
        runTest {
    ...
    @Test
    fun `saveSuspending - 기존 사용자를 업데이트한다`() =
        runTest {
            val original =
                mongoOperations.findOneOrNullSuspending<User>(
    ...
    @Test
    fun `findAsFlow - 조건에 맞는 사용자를 Flow로 조회한다`() =
        runTest {
            val users =
                mongoOperations
                    .findAsFlow<User>(
    ...
    @Test
    fun `findAllAsFlow - 전체 사용자를 Flow로 조회한다`() =
        runTest {
    ...
    @Test
    fun `findOneOrNullSuspending - 단건을 조회한다`() =
        runTest {
            val user =
                mongoOperations.findOneOrNullSuspending<User>(
    ...
    @Test
    fun `findOneOrNullSuspending - 결과가 없으면 null을 반환한다`() =
        runTest {
            val user =
                mongoOperations.findOneOrNullSuspending<User>(
    ...
    @Test
    fun `findByIdOrNullSuspending - ID로 사용자를 조회한다`() =
        runTest {
            val alice =
                mongoOperations.findOneOrNullSuspending<User>(
                    Criteria.where("name").`is`("Alice").toQuery()
    ...
    @Test
    fun `findByIdOrNullSuspending - 존재하지 않는 ID이면 null을 반환한다`() =
        runTest {
    ...
    @Test
    fun `findAsFlow - 오름차순으로 정렬하여 조회한다`() =
        runTest {
    ...
    @Test
    fun `findAsFlow - 내림차순으로 정렬하여 조회한다`() =
        runTest {
    ...
    @Test
    fun `findAsFlow - 페이지네이션을 적용하여 조회한다`() =
        runTest {
    ...
    @Test
    fun `countSuspending - 전체 사용자 수를 반환한다`() =
        runTest {
    ...
    @Test
    fun `countSuspending - 조건에 맞는 사용자 수를 반환한다`() =
        runTest {
            val count =
                mongoOperations.countSuspending<User>(
                    Query(Criteria.where("city").`is`("Seoul"))
                )
    ...
    @Test
    fun `existsSuspending - 조건에 맞는 문서가 존재하면 true를 반환한다`() =
        runTest {
            val exists =
    ...
    @Test
    fun `existsSuspending - 조건에 맞는 문서가 없으면 false를 반환한다`() =
        runTest {
            val exists =
                mongoOperations.existsSuspending<User>(
    ...
    @Test
    fun `updateFirstSuspending - 첫 번째 문서를 업데이트한다`() =
        runTest {
            val result =
                mongoOperations.updateFirstSuspending<User>(
                    Query(Criteria.where("city").`is`("Seoul")),
    ...
    @Test
    fun `updateMultiSuspending - 조건에 맞는 모든 문서를 업데이트한다`() =
        runTest {
            val result =
    ...
    @Test
    fun `upsertSuspending - 문서가 없으면 삽입한다`() =
        runTest {
            val update =
                ("name" setTo "NewUser")
            ...
            val result =
                mongoOperations.upsertSuspending<User>(
    ...
    @Test
    fun `removeSuspending(query) - 조건에 맞는 문서를 삭제한다`() =
        runTest {
            val result =
                mongoOperations.removeSuspending<User>(
                    Query(Criteria.where("name").`is`("Alice"))
                )
    ...
    @Test
    fun `findAndModifySuspending - 문서를 수정하고 수정 전 문서를 반환한다`() =
        runTest {
            val original =
                mongoOperations.findAndModifySuspending<User>(
                    Query(Criteria.where("name").`is`("Alice")),
                    Update().set("age", 31)
                )
            original.shouldNotBeNull()
    ...
    @Test
    fun `findAndRemoveSuspending - 문서를 삭제하고 삭제된 문서를 반환한다`() =
        runTest {
            val removed =
                mongoOperations.findAndRemoveSuspending<User>(
                    Query(Criteria.where("name").`is`("Alice"))
                )
    ...
    @Test
    fun `aggregateAsFlow - 도시별 사용자 수를 집계한다`() =
        runTest {
            val aggregation =
    ...
    @Test
    fun `criteria infix DSL - gt를 이용한 나이 조건 조회`() =
        runTest {
            val users =
                mongoOperations
                    .findAsFlow<User>(
    ...
    @Test
    fun `criteria infix DSL - eq를 이용한 이름 조건 조회`() =
        runTest {
            val user =
                mongoOperations.findOneOrNullSuspending<User>(
